  ...
  return [
    {
      ...
      value: [
        ...
        'Event family: static in-game calendar diagnostics',
        'Channel target: this command response only; no notification channel is configured or changed.',
        'Linked-clan scope: none; /events is server-safe and does not read or enroll linked clans.',
        `Active config counts: ${activeCount} active schedule windows, ${upcomingCount} upcoming schedule windows`,
        `Next transition: ${nextTransition ? formatNextTransition(nextTransition, view.timezone) : 'none'}`,
        'Persistence: response is derived in this server/DM context and does not write notification configs.',
        'Duplicates/not-found: not applicable; list output is rebuilt from the static schedule each run.',
        'Delivery: no worker/outbox fan-out is queued; only this Discord interaction is sent.',
