...
export class Util extends CocUtil {
    ...
    static dateToSerialDate(jsDate) {
    ...
    static clanGamesSeasonId() {
        const now = new Date();
        if (now.getDate() < 20)
            now.setMonth(now.getMonth() - 1);
        return now.toISOString().slice(0, 7);
    }
    static getClanGamesMaxPoints(season) {
        if ([0, 7, 11].includes(new Date(season || Date.now()).getMonth())) {
            return 5000;
        }
        return 4000;
    }
    static geRaidWeekend(now) {
    ...
    static raidWeekDateFormat(startDate, endDate) {
    ...
    static getCurrentLegendTimestamp() {
        const start = moment().hour() >= 5
    ...
    static getPreviousLegendTimestamp() {
     ...
     */
    static getLastSeasonId() {
        return Season.getLastSeason().seasonId;
    }
    static getCWLSeasonId() {
        return new Date().toISOString().slice(0, 7);
    }
    static escapeSheetName(name) {
        return name.replace(/[\*\?\:\[\]\\\/\']/g, '');
    }
    static delay(ms) {
    ...
    static _format(ms, msAbs, dur, longUnit, shortUnit, isLong = false) {
