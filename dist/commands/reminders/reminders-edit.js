...
export default class RemindersEditCommand extends Command {
    ...
    async clanWarsReminders(interaction, args) {
        const collection = this.client.db.collection("Reminders" /* Collections.WAR_REMINDERS */);
        ...
        const mutate = (disable = false) => {
            embed.setDescription([
                `**Edit War Reminder (${reminder.duration === 0 ? 'at the end' : `${this.getStatic(duration)} remaining`})** <#${reminder.channel}>`,
            ...
            const warTypeRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setDisabled(disable));
            const attackRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setDisabled(disable));
            const townHallRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setDisabled(disable));
            const clanRolesRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
        ...
        collector.on('collect', async (action) => {
            ...
            if (action.customId === customIds.save && action.isButton()) {
                ...
                const updated = await collection.findOneAndUpdate({ _id: reminder._id }, {
                    $set: {
    ...
    async clanGamesReminders(interaction, args) {
        const collection = this.client.db.collection("ClanGamesReminders" /* Collections.CLAN_GAMES_REMINDERS */);
        const reminders = await collection.find({ guild: interaction.guild.id }).toArray();
        if (!reminders.length)
            return interaction.editReply(this.i18n('command.reminders.no_reminders', { lng: interaction.locale }));
        const reminderId = reminders.find((rem) => hexToNanoId(rem._id) === args.id.toUpperCase())?._id;
        ...
        const mutate = (disable = false) => {
            ...
            const minPointsRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setDisabled(disable));
            const memberTypeRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    async capitalReminders(interaction, args) {
        const collection = this.client.db.collection("RaidReminders" /* Collections.RAID_REMINDERS */);
        const reminders = await collection.find({ guild: interaction.guild.id }).toArray();
        if (!reminders.length)
            return interaction.editReply(this.i18n('command.reminders.no_reminders', { lng: interaction.locale }));
        const reminderId = reminders.find((rem) => hexToNanoId(rem._id) === args.id.toUpperCase())?._id;
        ...
        const mutate = (disable = false) => {
            ...
            const remAttackRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setDisabled(disable));
            const minAttackRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
...
//# sourceMappingURL=reminders-edit.js.map