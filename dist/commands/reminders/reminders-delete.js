...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, escapeMarkdown, StringSelectMenuBuilder, time } from 'discord.js';
...
export default class RemindersDeleteCommand extends Command {
    ...
    async clanWarsReminders(interaction, args) {
        ...
        if (args.id) {
            ...
            await this.client.db
                .collection("Schedulers" /* Collections.WAR_SCHEDULERS */)
        ...
        const label = (duration) => moment.duration(duration).format('H[h], m[m], s[s]', { trim: 'both mid' });
        const state = {
            selected: null,
    ...
    async clanGamesReminders(interaction, args) {
        ...
        if (args.id) {
            ...
            await this.client.db
                ...
                .deleteOne({ _id: reminderId });
            await this.client.db
                .collection("ClanGamesSchedulers" /* Collections.CLAN_GAMES_SCHEDULERS */)
                .deleteMany({ reminderId });
            return interaction.editReply(this.i18n('command.reminders.delete.success', { lng: interaction.locale, id: args.id }));
        }
        if (reminders.length > 25)
        ...
        const embeds = () => {
            ...
            if (clanNames.length)
                ...
                embed.addFields([{ name: 'Clans', value: reminder.clans.join(', ').slice(0, 1024) }]);
    ...
    async capitalReminders(interaction, args) {
        ...
        if (args.id) {
            ...
            await this.client.db
                ...
                .deleteOne({ _id: reminderId });
            await this.client.db
                .collection("RaidSchedulers" /* Collections.RAID_SCHEDULERS */)
                .deleteMany({ reminderId });
            return interaction.editReply(this.i18n('command.reminders.delete.success', { lng: interaction.locale, id: args.id }));
        }
        if (reminders.length > 25)
...
//# sourceMappingURL=reminders-delete.js.map