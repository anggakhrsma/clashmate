import { CLAN_GAMES_MINIMUM_POINTS, MAX_TOWN_HALL_LEVEL } from '../../util/constants.js';
...
export default class RemindersNowCommand extends Command {
    ...
    async clanWarsReminders(interaction, args) {
        if (!args.message)
            return interaction.editReply(this.i18n('command.reminders.no_message', { lng: interaction.locale }));
        ...
        const mutate = (disable = false) => {
            const row0 = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setDisabled(disable));
            const row1 = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setDisabled(disable));
            const row2 = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                .setPlaceholder('Select Town Halls')
                .setCustomId(CUSTOM_ID.TOWN_HALLS)
                .setMaxValues(MAX_TOWN_HALL_LEVEL - 1)
                .setOptions(Array(MAX_TOWN_HALL_LEVEL - 1)
                .fill(0)
                .map((_, i) => {
        ...
        const collector = msg.createMessageComponentCollector({
            filter: (action) => Object.values(CUSTOM_ID).includes(action.customId) &&
        ...
        collector.on('end', async (_, reason) => {
            for (const id of Object.values(CUSTOM_ID))
                this.client.components.delete(id);
            if (!/delete/i.test(reason))
                await interaction.editReply({ components: mutate(true) });
        });
    }
    async clanGamesReminders(interaction, args) {
        ...
        const mutate = (disable = false) => {
            const row1 = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setOptions(CLAN_GAMES_MINIMUM_POINTS.map((num) => ({
                label: `${num}`,
                value: num.toString(),
    ...
    async capitalReminders(interaction, args) {
        ...
        const mutate = (disable = false) => {
            const minThresholdRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    async getClanWarsTexts(interaction, reminder) {
        const texts = [];
        const userIds = [];
        for (const tag of reminder.clans) {
            const currentWars = await this.client.coc.getCurrentWars(tag);
            for (const data of currentWars) {
                if (['notInWar', 'warEnded', 'preparation'].includes(data.state))
                    continue;
                const warType = data.warTag ? 'cwl' : data.isFriendly ? 'friendly' : 'normal';
                if (!reminder.warTypes.includes(warType))
                    continue;
                const [text, _userIds] = await this.client.clanWarScheduler.getReminderText({ ...reminder, guild: interaction.guild.id, smartSkip: false, linkedOnly: false }, { tag: data.clan.tag, warTag: data.warTag }, data);
                if (text) {
                    texts.push(text);
    ...
    async getClanGamesTexts(interaction, reminder) {
        const texts = [];
        const { endTime, startTime } = this.client.clanGamesScheduler.timings();
        if (!(Date.now() >= startTime && Date.now() <= endTime))
            return [];
        for (const tag of reminder.clans) {
            const [text] = await this.client.clanGamesScheduler.getReminderText({ ...reminder, guild: interaction.guild.id, linkedOnly: false }, { tag });
            if (text)
                texts.push(text);
        }
        return texts;
    }
    async getCapitalTexts(interaction, reminder) {
        const texts = [];
        for (const tag of reminder.clans) {
            const data = await this.client.capitalRaidScheduler.getLastRaidSeason(tag);
            if (!data)
                continue;
            const [text] = await this.client.capitalRaidScheduler.getReminderText({ ...reminder, guild: interaction.guild.id, linkedOnly: false }, { tag }, data);
            if (text)
                texts.push(text);
        }
        return texts;
    }
    async send(interaction, texts) {
...
//# sourceMappingURL=reminders-now.js.map