...
export default class ReorderCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        if (type === 'clans')
            return this.reorderClans(interaction, args);
        if (type === 'categories')
            return this.reorderCategories(interaction, args);
    }
    // ── Clans ──────────────────────────────────────────────────────────────────
    async reorderClans(interaction, args) {
        const col = this.client.db.collection("ClanStores" /* Collections.CLAN_STORES */);
        ...
        if (args.first && args.second) {
            ...
            // Save new order
            await Promise.all(clans.map((c, i) => col.updateOne({ _id: c._id }, { $set: { order: i + 1 } })));
            await this.client.settings.set(interaction.guildId, "clansSortingKey" /* Settings.CLANS_SORTING_KEY */, 'order');
        ...
        const updated = await col
            ...
            .toArray();
        if (!updated.length)
            return interaction.editReply('No clans linked to this server.');
        ...
        if (!args.first) {
            ...
            const row = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .setOptions(updated.slice(0, 25).map((c, i) => ({
                label: `${i + 1}. ${c.name}`,
                value: c.tag,
                description: c.tag
            }))));
        ...
        const row = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
            .setCustomId(this.createId({ cmd: this.id, type: 'clans', first: args.first, string_key: 'second' }))
            .setPlaceholder(`Move "${clanName}" to position...`)
            .setOptions(positions.slice(0, 25)));
        return interaction.editReply({ embeds: [embed], components: [row] });
    }
    // ── Categories ─────────────────────────────────────────────────────────────
    async reorderCategories(interaction, args) {
        const col = this.client.db.collection("ClanCategories" /* Collections.CLAN_CATEGORIES */);
        if (args.first && args.second) {
            ...
            await Promise.all(cats.map((c, i) => col.updateOne({ _id: c._id }, { $set: { order: i + 1 } })));
            args.first = undefined; // reset to step 1
        }
