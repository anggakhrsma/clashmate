...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, DiscordjsError, DiscordjsErrorCodes, EmbedBuilder, ModalBuilder, PermissionFlagsBits, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
import { Command } from '../../lib/handlers.js';
import { eventsMap, imageMaps, locationsMap } from '../../struct/guild-events-handler.js';
export default class SetupEventsCommand extends Command {
    ...
    async exec(interaction, { disable }) {
        if (disable) {
            await this.client.db
                .collection("GuildEvents" /* Collections.GUILD_EVENTS */)
        ...
        const menu = new StringSelectMenuBuilder()
            ...
            .setOptions(this.client.guildEvents.eventTypes.map((id) => ({
            label: eventsMap[id],
            value: id,
        ...
        const durationMenu = new StringSelectMenuBuilder()
            ...
            .setOptions(durationEvents.map((id) => ({
            label: eventsMap[id],
            value: id,
        ...
        const durationRow = new ActionRowBuilder().setComponents(durationMenu);
        const buttonRow = new ActionRowBuilder().setComponents(new ButtonBuilder()
        ...
        const getEmbed = (creating = true) => {
            ...
            embed.setDescription([
                ...
                ...this.client.guildEvents.eventTypes
                    ...
                    .map((event) => {
                    const eventName = state[imageMaps[event]]
                        ? `[${eventsMap[event]}](<${state[imageMaps[event]]}>)`
                        : `${eventsMap[event]}`;
                    const location = state[locationsMap[event]]
                        ? `${state[locationsMap[event]]}`
                        : null;
        ...
        collector.on('collect', async (action) => {
            ...
            if (action.customId === customIds.select && action.isStringSelectMenu()) {
                ...
                menu.setOptions(this.client.guildEvents.eventTypes.map((id) => ({
                    label: eventsMap[id],
                    value: id,
                    description: `The event stays until the end of the ${eventsMap[id]}`,
                    default: state.allowedEvents.includes(id)
            ...
            if (action.customId === customIds.duration && action.isStringSelectMenu()) {
                ...
                durationMenu.setOptions(durationEvents.map((id) => ({
                    label: eventsMap[id],
                    value: id,
                    default: state.durationOverrides.includes(id)
                })));
            ...
            if (action.customId === customIds.images) {
                ...
                const clanGamesImageInput = new TextInputBuilder()
                    ...
                    .setMaxLength(256)
                    .setRequired(false);
                if (state.clan_games_image_url)
                    clanGamesImageInput.setValue(state.clan_games_image_url);
                const cwlImageInput = new TextInputBuilder()
                    ...
                    .setMaxLength(256)
                    .setRequired(false);
                if (state.cwl_image_url)
                    cwlImageInput.setValue(state.cwl_image_url);
                const capitalRaidImageInput = new TextInputBuilder()
                    ...
                    .setMaxLength(256)
                    .setRequired(false);
                if (state.raid_week_image_url)
                    capitalRaidImageInput.setValue(state.raid_week_image_url);
                modal.addComponents(new ActionRowBuilder().addComponents(seasonResetImageInput), new ActionRowBuilder().addComponents(cwlImageInput), new ActionRowBuilder().addComponents(capitalRaidImageInput), new ActionRowBuilder().addComponents(clanGamesImageInput));
            ...
            if (action.customId === customIds.locations) {
                ...
                const clanGamesLocationInput = new TextInputBuilder()
                    ...
                    .setMaxLength(100)
                    .setRequired(false);
                if (state.clan_games_location)
                    clanGamesLocationInput.setValue(state.clan_games_location);
                const cwlLocationInput = new TextInputBuilder()
                    ...
                    .setMaxLength(100)
                    .setRequired(false);
                if (state.cwl_location)
                    cwlLocationInput.setValue(state.cwl_location);
                const capitalRaidLocationInput = new TextInputBuilder()
                    ...
                    .setMaxLength(100)
                    .setRequired(false);
                if (state.raid_week_location)
                    capitalRaidLocationInput.setValue(state.raid_week_location);
                modal.addComponents(new ActionRowBuilder().addComponents(seasonResetLocationInput), new ActionRowBuilder().addComponents(cwlLocationInput), new ActionRowBuilder().addComponents(capitalRaidLocationInput), new ActionRowBuilder().addComponents(clanGamesLocationInput));
                await action.showModal(modal);
                try {
                    const modalSubmit = await action.awaitModalSubmit({
                        time: 10 * 60 * 1000,
...
//# sourceMappingURL=setup-events.js.map