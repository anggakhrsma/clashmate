import { DEEP_LINK_TYPES, FeatureFlags, missingPermissions, URL_REGEX } from '../../util/constants.js';
import { ClanLogType } from '../../entities/index.js';
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, DiscordjsError, DiscordjsErrorCodes, messageLink, ModalBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, WebhookClient } from 'discord.js';
import { ObjectId } from 'mongodb';
import { clanEmbedMaker } from '../../helper/clan-embed.helper.js';
import { Command } from '../../lib/handlers.js';
import { ClanEmbedFieldOptions } from '../../util/command.options.js';
...
export default class ClanEmbedCommand extends Command {
    ...
    async exec(interaction, args) {
        if (args.disable_embed) {
            ...
            await this.client.db.collection("ClanLogs" /* Collections.CLAN_LOGS */).deleteMany({ _id: { $in: logIds } });
        ...
        const existing = await this.client.db.collection("ClanLogs" /* Collections.CLAN_LOGS */).findOne({
            clanTag: data.tag,
        ...
        const menu = new StringSelectMenuBuilder()
            .setOptions(ClanEmbedFieldOptions.map(({ label, value }) => ({
            label,
            value,
        ...
        const buttonRow = new ActionRowBuilder().addComponents(customizeButton, confirmButton);
        ...
        const onFieldsCustomization = async (action) => {
            await action.deferUpdate();
            state.fields = action.values;
            menu.setOptions(ClanEmbedFieldOptions.map(({ label, value }) => ({
                label,
                value,
                default: state.fields?.includes(value)
            })));
        ...
        const onCustomization = async (action) => {
            ...
            modal.addComponents(new ActionRowBuilder().addComponents(descriptionInput), new ActionRowBuilder().addComponents(requirementsInput), new ActionRowBuilder().addComponents(bannerImageInput));
        ...
        const mutate = async (messageId, channelId, webhook) => {
            const id = await this.client.storage.register(interaction, {
                op: 8 /* Flags.CLAN_EMBED_LOG */,
            ...
            await this.client.db.collection("ClanLogs" /* Collections.CLAN_LOGS */).updateOne({
                clanTag: data.tag,
                guildId: interaction.guildId,
                logType: ClanLogType.CLAN_EMBED_LOG
            }, {
        ...
        const getEmbedButtons = () => {
            ...
            return [new ActionRowBuilder().addComponents(rulesButton)];
        };
        const onResend = async (action) => {
            const msg = await webhook.send(channel.isThread()
        ...
        const onConfirm = async (action) => {
            try {
                if (!existing?.messageId)
                    throw new Error('No existing embed found', { cause: '6969' });
                const channel = interaction.guild.channels.cache.get(existing.channelId);
                const webhook = new WebhookClient(existing.webhook);
                const msg = await webhook.editMessage(existing.messageId, channel?.isThread()
                    ? { embeds: [embed], threadId: channel.id, components: getEmbedButtons() }
                    : { embeds: [embed], components: getEmbedButtons() });
...
//# sourceMappingURL=setup-clan-embed.js.map