...
export default class SetupListCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
            ...
            .setLabel('Linked Clans'), new ButtonBuilder()
        ...
        if ((args.clans && clans.length) || args.logs) {
            ...
            if (!embeds.length)
                return interaction.editReply('No clans are linked.');
    ...
    async getClanList(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .map((clan) => `\`\u200e${clan.name.padEnd(nameLen, ' ')} ${clan.tag.padStart(tagLen, ' ')}  ${clan.members
            .toString()
            .padStart(2, ' ')}/50 \u200f\``)
            .join('\n'));
        return [embed];
    }
    async getFeatures(interaction, clans) {
        ...
        return fetched.map((clan) => {
            ...
            if (channels.length)
                embed.setDescription(channels.join(', '));
            embed.addFields(clan.features.map((record) => ({
                name: record.name,
...
//# sourceMappingURL=setup-list.js.map