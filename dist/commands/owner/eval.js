...
export default class EvalCommand extends Command {
    ...
    async exec(interaction, { code, depth, shard }) {
        let hrDiff;
        let evaled;
        try {
            const hrStart = process.hrtime();
            evaled = await (shard ? Promise.resolve([eval(code)]) : eval(code));
        ...
        return interaction.followUp({ content: result, flags: MessageFlags.Ephemeral });
    }
    _result(result, hrDiff, depth, shard) {
        ...
        const prependPart = !inspected.startsWith('{') && !inspected.startsWith('[') && !inspected.startsWith("'")
            ? split[0]
            : inspected[0];
