import { UNRANKED_WAR_LEAGUE_ID, WAR_LEAGUE_PROMOTION_MAP } from '../../util/constants.js';
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, escapeMarkdown, MessageFlags, StringSelectMenuBuilder } from 'discord.js';
...
export default class CWLStarsCommand extends Command {
    ...
    async rounds(interaction, { body, clan, args }) {
        ...
        for (const data of body.wars) {
            if (data.clan.tag === clanTag || data.opponent.tag === clanTag) {
                ...
                if (['inWar', 'warEnded'].includes(data.state)) {
                    if (this.client.coc.isWinner(clan, opponent))
                        warsWon++;
                    for (const m of clan.members) {
                        if (m.attacks && !m.attacks.length && !m.bestOpponentAttack && body.fromArchive)
                            continue;
                        members[m.tag] ??= {
                            name: m.name || m.tag,
                            tag: m.tag,
        ...
        const bonuses = WAR_LEAGUE_PROMOTION_MAP[(leagueId || clan.warLeague?.id) ?? UNRANKED_WAR_LEAGUE_ID].bonuses +
            warsWon;
        ...
        embed.setFooter({
            text: `CWL ${moment(body.season).format('MMM YYYY')} | ${bonuses} Bonuses Assignable`
        });
        const payload = {
            cmd: this.id,
            tag: clanTag,
        ...
        return interaction.editReply({
            embeds: [embed],
            components: clanMenu ? [buttonRow, menuRow, clanMenu] : [buttonRow, menuRow]
        });
    }
}
//# sourceMappingURL=cwl-stars.js.map