import { calculateCWLMedals, WAR_LEAGUE_PROMOTION_MAP, WAR_LEAGUE_MAP } from '../../util/constants.js';
...
export default class CWLStatsCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const liveLeagues = res.ok ? (body.leagues ?? {}) : {};
        const aggregated = await this.client.coc.aggregateClanWarLeague(clan.tag, { ...entityLike, leagues: Object.keys(liveLeagues).length ? liveLeagues : (group?.leagues ?? {}) }, isApiData);
        if (!aggregated) {
            return interaction.followUp({
                flags: MessageFlags.Ephemeral,
                content: this.i18n('command.cwl.not_in_season', {
    ...
    async rounds(interaction, { body, clan, args }) {
        ...
        if (leagueId) {
            ...
            embed.setDescription([
                ...
                ranks
                    .map((clan) => {
                    const emoji = clan.rank <= WAR_LEAGUE_PROMOTION_MAP[leagueId].promotion
                        ? EMOJIS.UP_KEY
                        : clan.rank >= WAR_LEAGUE_PROMOTION_MAP[leagueId].demotion
                            ? EMOJIS.DOWN_KEY
                            : EMOJIS.STAYED_SAME;
                    return `${emoji}${BLUE_NUMBERS[clan.rank]} \`\u200e ${clan.stars.toString().padEnd(3, ' ')} ${this.dest(clan.destruction, padding)}  ${Util.escapeBackTick(clan.name).padEnd(15, ' ')}\``;
                })
                    .join('\n'),
                '',
            ...
            embed.setDescription([
                ...
                ranks
                    .map((clan, i) => {
                    return `${BLUE_NUMBERS[++i]} \`\u200e ${padEnd(clan.stars, 3)} ${this.dest(clan.destruction, padding)}  ${Util.escapeBackTick(clan.name).padEnd(15, ' ')}\``;
                })
                    .join('\n'),
                '',
        ...
        try {
            ...
            const imageBuffer = await fetch(`${process.env.IMAGE_GEN_API_BASE_URL}/v1/cwl/generate-ranking-image`, {
                method: 'POST',
                headers: {
...
//# sourceMappingURL=cwl-stats.js.map