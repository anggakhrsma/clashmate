...
export default class FlagDeleteCommand extends Command {
    ...
    async exec(interaction, args) {
        if (!args.player)
            return interaction.editReply(this.i18n('command.flag.delete.no_tag', { lng: interaction.locale }));
        const playerTag = this.client.coc.fixTag(args.player);
        const playerTags = [];
        ...
        const collection = this.client.db.collection("Flags" /* Collections.FLAGS */);
        ...
        return interaction.editReply(this.i18n('command.flag.delete.success', {
            lng: interaction.locale,
            tag: `${playerTag}${refId ? ` and ref \`${refId}\`` : ''}`
        }));
    }
}
//# sourceMappingURL=flag-delete.js.map