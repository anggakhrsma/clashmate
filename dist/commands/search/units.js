...
export default class UnitsCommand extends Command {
    ...
    embed(data, showMaxLevel = false, equipmentOnly = false) {
        ...
        const Troops = RAW_TROOPS_WITH_ICONS.filter((troop) => !troop.seasonal && !(troop.name in SUPER_TROOPS))
            .filter((unit) => {
            if (unit.category === 'equipment' && !unlockedEquipment.includes(unit.name))
                return false;
        ...
        if (equipmentOnly)
            return embed;
        const superTroops = RAW_SUPER_TROOPS.filter((unit) => apiTroops.find((un) => un.name === unit.original &&
            un.village === unit.village &&
            un.level >= unit.minOriginalLevel)).map((unit) => {
            ...
            embed.addFields([
                {
                    ...
                    value: [
                        cluster(superTroops.filter((en) => activeSuperTroops.length ? activeSuperTroops.includes(en.name) : true), 4)
                            .map((chunks) => chunks
                            .map((unit) => {
