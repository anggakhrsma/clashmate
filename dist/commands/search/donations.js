...
export default class DonationsCommand extends Command {
    ...
    async exec(interaction, args) {
        if (args.start_date) {
            return this.handler.exec(interaction, this.handler.getCommand('donations-range'), args);
        }
        ...
        const dbMembers = await this.client.db
            ...
            .find({ season, __clans: clan.tag, tag: { $in: clan.memberList.map((m) => m.tag) } }, { projection: { tag: 1, clans: 1, townHallLevel: 1 } })
            .toArray();
