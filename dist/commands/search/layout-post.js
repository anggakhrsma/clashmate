import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonStyle, ComponentType, DiscordjsError, DiscordjsErrorCodes, MessageFlags, ModalBuilder, PermissionFlagsBits, TextDisplayBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
...
export default class LayoutCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        if (layoutTemplate && !args.notes)
            args.notes = layoutTemplate;
        if (interaction.isCommand()) {
            return this.handleSubmit(interaction, args);
        }
        const isAdmin = this.client.util.isManager(interaction.member) ||
        ...
        if (layout)
            args.downloads = layout.downloader.length;
    ...
    async handleSubmit(interaction, args) {
        ...
        await this.collection.updateOne({ layoutId, guildId: interaction.guild.id }, {
            $set: shake({
        ...
        const isVotingEnabled = this.client.settings.get(interaction.guild, "allowLayoutVoting" /* Settings.ALLOW_LAYOUT_VOTING */, false);
    ...
    async displayLink(interaction, args) {
        const updated = await this.collection.findOneAndUpdate({ messageIds: interaction.message.id, guildId: interaction.guild.id }, { $addToSet: { downloader: interaction.user.id } }, { returnDocument: 'after' });
        if (!updated)
            return interaction.reply({
                flags: MessageFlags.Ephemeral,
    ...
    async viewDownloader(interaction) {
        const layout = await this.collection.findOne({
            messageIds: interaction.message.id,
            guildId: interaction.guild.id
        });
    ...
    getUrlFromInteractionComponents(interaction) {
...
//# sourceMappingURL=layout-post.js.map