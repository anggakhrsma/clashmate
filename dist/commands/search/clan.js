import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, escapeMarkdown, StringSelectMenuBuilder } from 'discord.js';
...
export default class ClanCommand extends Command {
    ...
    async exec(interaction, args) {
        if (args.by_player_tag && !args.tag) {
            ...
            if (res.ok && player.clan)
                args.tag = player.clan.tag;
        }
        ...
        const menu = new StringSelectMenuBuilder()
            ...
            .addOptions(Object.values(MembersCommandOptions).map((option) => ({
            label: option.label,
            value: option.id,
    ...
    async embed(guildId, clan) {
        const embed = new EmbedBuilder()
            .setTitle(`${escapeMarkdown(clan.name)} (${clan.tag})`)
            .setURL(this.client.coc.getClanURL(clan.tag))
        ...
        embed.addFields([
            {
                ...
                value: [
                    ...
                    `${location}${rank}`,
                    '**Requirements**',
                    `⚙️ ${clanTypes[clan.type]} ${EMOJIS.TROPHY} ${clan.requiredTrophies} Required ${clan.requiredTownhallLevel ? `${EMOJIS.TOWN_HALL} ${clan.requiredTownhallLevel}+` : ''}`,
                    '\u200b\u2002'
                ].join('\n')
            }
        ]);
        ...
        embed.addFields([
            {
                ...
                value: [
                    ...
                    '**War Performance**',
                    `${EMOJIS.OK} ${clan.warWins} Won ${clan.isWarLogPublic
    ...
    async getActivity(clan) {
        ...
        const rows = await this.client.db
            ...
            .toArray();
        if (!rows.length)
            return { avgDailyActivity: 0, avgDailyOnline: 0 };
        const avgDailyActivity = rows.reduce((s, r) => s + r.count, 0) / rows.length;
        const avgDailyOnline = rows.reduce((s, r) => s + r.uniqueMembers, 0) / rows.length;
        return {
            avgDailyActivity: Number(avgDailyActivity.toFixed()),
            avgDailyOnline: Number(avgDailyOnline.toFixed())
        };
    }
    async getSeason(clan) {
