...
import { DARK_ELIXIR_TROOPS, DARK_SPELLS, ELIXIR_SPELLS, ELIXIR_TROOPS, EMOJIS, HERO_EQUIPMENT, HERO_PETS, HOME_HEROES, SIEGE_MACHINES, SUPER_TROOPS } from '../../util/emojis.js';
...
export default class ArmyCommand extends Command {
    ...
    embed(guild, locale, args) {
        const url = this.getURL(args.link ?? args.message);
        const army = url?.searchParams.get('army');
        if (!army)
            return { embeds: [], content: this.i18n('command.army.no_link', { lng: locale }) };
        const troopParts = [];
        const spellParts = [];
        const heroParts = [];
        const ccTroopParts = [];
        ...
        const parseArmyCombinations = (combinations) => {
            return combinations
        ...
        const parseHeroCombinations = (combinations) => {
            const heroes = [];
        ...
        const malformed = ![...troopList, ...spellList].every((en) => typeof en.id === 'number' && typeof en.total === 'number' && en.total <= TOTAL_UNITS);
        if (malformed)
            return { embeds: [], content: this.i18n('command.army.invalid_link', { lng: locale }) };
        const SPELLS = {
            ...DARK_SPELLS,
        ...
        const findUnits = ({ emojiRecords, category, subCategory, parts }) => {
            const _findOne = (id) => {
                return RAW_TROOPS.find((en) => en.id === id &&
        ...
        const hallByUnlockTH = Math.max(...troops.map((en) => en.hallLevel), ...spells.map((en) => en.hallLevel), ...siegeMachines.map((en) => en.hallLevel), ...superTroops.map((en) => en.hallLevel));
            ...
            embed.addFields([
                {
                    ...
                    value: [
                        '**Super Troops**',
                        superTroops
                            .map((en) => `\u200e\`${this.padding(en.total)}\` ${SUPER_TROOPS[en.name]}  ${en.name}`)
                            .join('\n')
                    ].join('\n')
                }
            ]);
            ...
            embed.addFields([
                {
                    ...
                    value: [
                        '**Siege Machines**',
                        siegeMachines
                            .map((en) => `\u200e\`${this.padding(en.total)}\` ${SIEGE_MACHINES[en.name]}  ${en.name}`)
                            .join('\n')
                    ].join('\n')
                }
            ]);
