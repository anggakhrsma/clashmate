...
export default class ClanSearchCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        row.setComponents(this.paginatedMenu(items, 0, customIds).setPlaceholder(`Page ${1} of ${Math.ceil(items.length / 25)} (Total: ${items.length})`));
        ...
        createInteractionCollector({
            ...
            onSelect: async (action) => {
                ...
                const menu = this.paginatedMenu(items, pageIndex, customIds).setPlaceholder(`Page ${pageIndex + 1} of ${Math.ceil(items.length / 25)} (Total: ${items.length})`);
                newRow.setComponents(menu);
                await action.update({ components: [newRow] });
            }
        });
    }
    paginatedMenu(items, pageIndex, customIds) {
