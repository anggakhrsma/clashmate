...
export default class RemainingCommand extends Command {
    ...
    sendResult(interaction, body) {
        ...
        if (TwoRem.length) {
            embed.setDescription([
                ...
                ...TwoRem.sort((a, b) => a.mapPosition - b.mapPosition).map((m) => `\u200e${BLUE_NUMBERS[m.mapPosition]} ${escapeMarkdown(m.name)}`)
            ].join('\n'));
        }
        if (OneRem.length && body.attacksPerMember !== 1) {
            embed.setDescription([
                ...
                ...OneRem.sort((a, b) => a.mapPosition - b.mapPosition).map((m) => `\u200e${BLUE_NUMBERS[m.mapPosition]} ${escapeMarkdown(m.name)}`)
            ].join('\n'));
        }
        if (body.id)
    ...
    async forUsers(interaction, args) {
        const player = args.player_tag
            ? await this.client.resolver.resolvePlayer(interaction, args.player_tag)
            : null;
        if (args.player_tag && !player)
            return null;
        args.is_grouped =
            typeof args.is_grouped === 'boolean'
                ? args.is_grouped
                : this.client.settings.get(interaction.guildId, "useGroupedTodoList" /* Settings.USE_GROUPED_TODO_LIST */, true);
        const embeds = [];
        switch (args.type) {
    ...
    async warAttacks(interaction, args) {
        ...
        for (const data of wars) {
            ...
            for (const tag of playerTags) {
                ...
                if (attacks.length === data.attacksPerMember)
                    continue;
                players.push({
                    member,
                    clan,
        ...
        const groupedDescription = Object.entries(group(players, (player) => player.clan.tag))
            ...
            return [
                ...
                wars
                    .map(({ member, endTime, remaining }) => `- ${escapeMarkdown(member.name)} (${member.tag})\n - ${remaining} remaining (${time(endTime, 'R')})`)
                    .join('\n')
            ];
        })
            .join('\n');
        const listedDescription = players
            ...
            .join('\n\n');
        if (args.is_grouped)
            embed.setDescription(groupedDescription || null);
        else
            embed.setDescription(listedDescription || null);
        const remaining = players.reduce((a, b) => a + b.remaining, 0);
        embed.setFooter({ text: `${remaining} Remaining` });
        return embed;
    }
    async capitalRaids(interaction, args) {
        const playerTags = args.player
            ? [args.player.tag]
            : await this.client.resolver.getLinkedPlayerTags(args.user.id);
        ...
        for (const raid of raids) {
            for (const playerTag of playerTags) {
                ...
                if (!raidMember)
                    continue;
                if (raidMember.attackLimit + raidMember.bonusAttackLimit === raidMember.attacks)
                    continue;
                players.push({
                    clan: {
                        name: raid.name,
        ...
        const groupedDescription = Object.entries(group(players, (player) => player.clan.tag))
            ...
            return [
                `### ${escapeMarkdown(clan.name)} (${clan.tag})`,
                raids
                    .map(({ member, endTime, attackLimit, attacks }) => `- ${escapeMarkdown(member.name)} (${member.tag})\n - ${attacks}/${attackLimit} raids (${time(endTime, 'R')})`)
                    .join('\n')
            ];
        })
            .join('\n');
    ...
    async clanGames(interaction, args) {
        const playerTags = args.player
            ? [args.player.tag]
            : await this.client.resolver.getLinkedPlayerTags(args.user.id);
        ...
        for (const member of members) {
            for (const playerTag of playerTags) {
                if (member.tag !== playerTag)
                    continue;
                if (member.current - member.initial >= maxPoints)
                    continue;
                players.push({
                    clan: {
    ...
    async legendAttacks(interaction, args) {
        const playerTags = args.player
            ? [args.player.tag]
