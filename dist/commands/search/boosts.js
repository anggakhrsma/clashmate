...
export default class BoostsCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const recently = boostTimes
            .filter((m) => m.lastSeen >= new Date(Date.now() - 10 * 60 * 1000))
            .map((m) => m.tag);
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`**Currently Boosted Super Troops**${args.recent && recently.length ? '\nRecently Active Members (~10m)' : ''}\n\u200b`);
            ...
            embed.addFields([
                {
                    ...
                    value: Util.splitMessage(`${val
                        .map((mem) => `\u200e${mem.name}${mem.duration ? ` (${Util.duration(mem.duration)})` : ''} ${mem.online ? EMOJIS.ONLINE : ''}`)
                        .join('\n')}\n\u200b`, { maxLength: 1024 })[0]
                }
            ]);
            embed.setTimestamp();
        ...
        const menus = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
            ...
            .addOptions(Object.entries(SUPER_TROOPS).map(([key, value]) => ({
            label: key,
            value: key,
            emoji: value
        }))));
    ...
    boostable(players) {
        return players
            ...
            .reduce((pre, curr) => {
            const troops = RAW_SUPER_TROOPS.filter((unit) => curr.troops.find((un) => un.village === 'home' &&
