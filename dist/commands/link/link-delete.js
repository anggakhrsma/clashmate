...
export default class LinkDeleteCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        if (clanTag) {
            ...
            if (unlinked) {
                return interaction.editReply(this.i18n('command.link.delete.success', {
        ...
        if (!(interaction.user.id === member.id || linkedByUserIds.includes(member.linkedBy)) &&
            ...
            if (!players.length) {
                return interaction.editReply(this.i18n('command.link.delete.no_access', {
                    lng: interaction.locale,
    ...
    async getMember(tag, interaction) {
        ...
        if (target &&
            link &&
            link.userId !== target.userId &&
            [link.userId, target.userId].includes(interaction.user.id)) {
