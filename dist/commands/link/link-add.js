import { ActionRowBuilder, DiscordjsError, DiscordjsErrorCodes, MessageFlags, ModalBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
...
export default class LinkAddCommand extends Command {
    ...
    async exec(interaction, { token_field }) {
        if (interaction.isButton())
            return this.modal(interaction, token_field);
    }
    async playerLink(interaction, { player, member }) {
    ...
    async modal(interaction, token_field) {
        const customIds = {
    ...
    async verify(interaction, data, token) {
