...
export default class LinkListCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        if (args.links) {
            if (!this.client.util.isManager(interaction.member, "linksManagerRole" /* Settings.LINKS_MANAGER_ROLE */)) {
            ...
            const linkRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
                .setURL(`https://clashperk.com/links?tag=${encodeURIComponent(clan.tag)}&bot=${'public'}&token=${token}`)
        ...
        if (args.sort_by)
            embed.setFooter({ text: `Sorted by ${title(args.sort_by)}` });
