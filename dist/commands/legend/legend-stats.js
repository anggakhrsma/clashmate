import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, MediaGalleryBuilder, MediaGalleryItemBuilder, MessageFlags, StringSelectMenuBuilder, TextDisplayBuilder, time } from 'discord.js';
...
export default class LegendStatsCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const thresholdRecords = moment().diff(startTime, 'days') >= minLogs
        ...
        const menu = new StringSelectMenuBuilder()
            ...
            .setOptions(possibleRanks.map((rank) => ({
            label: `#${rank}`,
            value: rank.toString(),
    ...
    async getLegendThreshold(isEod, ref) {
    ...
    compare(target, reference) {
        ...
        const thresholds = target.thresholds.map((threshold) => {
            const eod = reference?.thresholds.find((t) => t.rank === threshold.rank)?.minTrophies ??
...
//# sourceMappingURL=legend-stats.js.map