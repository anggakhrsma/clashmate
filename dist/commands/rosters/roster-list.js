...
import { ROSTER_MAX_LIMIT, rosterLabel } from '../../struct/roster-manager.js';
export default class RosterListCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        if (args.user)
            query['members.userId'] = args.user.id;
        else if (args.player)
            query['members.tag'] = args.player;
        if (args.name) {
            const text = args.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            query.name = { $regex: `.*${text}.*`, $options: 'i' };
        }
        if (args.clan)
            query['clan.tag'] = args.clan;
        if (args.category)
        ...
        const rosterEmbed = new EmbedBuilder()
            ...
            .setTitle('Rosters');
        if (rosters.length) {
            rosterEmbed.setDescription(rosters
                .map((roster, i) => {
            ...
            if (args.user) {
                const members = rosters
                    .map((roster) => roster.members.map((member) => ({
                    name: member.name,
                    tag: member.tag,
                ...
                rosterEmbed.setDescription(groupedMembers
                    .map((member) => {
                    return [
        ...
        if (isQuery)
            rosterEmbed.setFooter({ text: 'Search Results' });
        if (rosters.length)
            embeds.push(rosterEmbed);
        if (isQuery) {
            if (!embeds.length)
                return interaction.editReply({ content: 'No rosters found.' });
        ...
        if (categories.length) {
            groupEmbed.setDescription(categories
                .map((category, i) => {
        ...
        if (!embeds.length)
            return interaction.editReply({ content: 'No rosters or groups found.' });
        return interaction.editReply({ embeds });
    }
}
//# sourceMappingURL=roster-list.js.map