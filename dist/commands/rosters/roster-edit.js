import { UNRANKED_WAR_LEAGUE_ID, URL_REGEX } from '../../util/constants.js';
...
import { rosterLabel, rosterLayoutMap } from '../../struct/roster-manager.js';
...
export default class RosterEditCommand extends Command {
    ...
    async exec(interaction, args) {
        if (args.roster === '*')
            return this.handleBulk(interaction);
        ...
        if (args.roster_role) {
            ...
            if (dup)
                return interaction.editReply({ content: 'A roster with this role already exists.' });
        }
        const data = {};
        if (args.clan && clan)
            data.clan = {
        ...
        if (args.name)
            data.name = args.name;
        if (isNumber(args.max_members))
            data.maxMembers = args.max_members;
        if (isNumber(args.min_town_hall))
            data.minTownHall = args.min_town_hall;
        if (isNumber(args.max_town_hall))
            data.maxTownHall = args.max_town_hall;
        if (isNumber(args.min_hero_level))
            data.minHeroLevels = args.min_hero_level;
        if (args.roster_role)
            data.roleId = args.roster_role.id;
        if (args.delete_role)
            data.roleId = null;
        if (args.roster_image_url && URL_REGEX.test(args.roster_image_url))
            data.rosterImage = args.roster_image_url;
        if (args.roster_image_url && /^none$/i.test(args.roster_image_url))
            data.rosterImage = null;
        ...
        if (typeof args.use_clan_alias === 'boolean')
            data.useClanAlias = args.use_clan_alias;
        if (typeof args.allow_unlinked === 'boolean')
            data.allowUnlinked = args.allow_unlinked;
        if (typeof args.color_code === 'number')
            data.colorCode = args.color_code;
        if (args.category)
        ...
        const url = `https://clashperk.com/rosters?roster=${updated._id.toHexString()}&bot=${false ? 'custom' : 'public'}&token=${token}`;
    ...
    async handleBulk(interaction) {
        ...
        const layoutMenu = new StringSelectMenuBuilder()
            ...
            .setMaxValues(5)
            .setOptions(keys.map(([key, { name, description }]) => ({ label: name, description, value: key })));
        const layoutMenuRow = new ActionRowBuilder().addComponents(layoutMenu);
        ...
        const sortMenuRow = new ActionRowBuilder().addComponents(sortMenu);
        const applyToAllButton = new ButtonBuilder()
            .setCustomId(customIds.applyToAll)
            .setStyle(ButtonStyle.Danger)
            .setLabel('Apply to all Rosters')
            .setDisabled(true);
        const applyRow = new ActionRowBuilder().addComponents(applyToAllButton);
        const selected = {};
        const getContent = () => {
            const texts = [
        ...
        const selectSortBy = async (action) => {
            selected.sortBy = action.values.at(0);
        ...
        const selectLayout = async (action) => {
...
//# sourceMappingURL=roster-edit.js.map