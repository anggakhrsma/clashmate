import { TAG_REGEX } from '../../util/constants.js';
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, DiscordjsError, DiscordjsErrorCodes, MessageFlags, ModalBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, UserSelectMenuBuilder } from 'discord.js';
import { ObjectId } from 'mongodb';
import pluralize from 'pluralize';
import { cluster, unique } from 'radash';
import { Command } from '../../lib/handlers.js';
import { ROSTER_MAX_LIMIT, rosterClan, rosterLabel, RosterLog } from '../../struct/roster-manager.js';
...
export default class RosterManageCommand extends Command {
    ...
    async autocomplete(interaction, args) {
        ...
        if (!args.action)
            return interaction.respond([{ name: 'No action was selected.', value: '0' }]);
        ...
        if (focused.name === 'target_roster') {
            ...
            return interaction.respond(rosters.map((roster) => ({
        ...
        if (focused.name === 'target_group') {
            ...
            if (!roster)
                return interaction.respond([{ name: 'Roster was deleted.', value: '0' }]);
            const filter = {
                guildId: interaction.guild.id
            };
            ...
            if (categoryId && args.action === 'change-category')
                filter._id = { $ne: categoryId };
            ...
            return interaction.respond(categories.map((category) => ({
    ...
    async exec(interaction, args) {
        ...
        if (args.action === 'change-roster') {
            ...
            if (!rosterMember)
                return interaction.editReply({ content: 'User not found in roster.' });
            ...
            if (!swapped.success)
                return interaction.editReply({ content: swapped.message });
    ...
    async changeCategory(interaction, { roster, playerTag, user, categoryId }) {
        if (categoryId && !ObjectId.isValid(categoryId))
            return interaction.editReply({ content: 'Invalid target group ID.' });
        const playerCustomIds = {
        ...
        if (playerTag)
            selected.playerTags.push(playerTag);
        if (user) {
            selected.user = user;
        ...
        if (!categories.length)
            return interaction.editReply({ content: 'No user groups found.' });
        const maxItems = 25;
        const getOptions = () => {
        ...
        const buttonRow = new ActionRowBuilder().addComponents(confirmButton, categorySelectButton, deselectButton);
        const headerTexts = [
            '**Changing User Group**',
            '',
        ...
        const confirm = async (action) => {
            ...
            const players = await this.client.rosterManager.getClanMemberLinks(selected.playerTags.map((tag) => ({ tag })), true);
            const changeLog = [];
            for (const player of players) {
                ...
                if (newRoster) {
                    ...
                    if (member)
                        changeLog.push({ ...member, categoryId: oldMember?.categoryId });
                }
            }
            if (changeLog.length) {
        ...
        const playerTagsMap = {};
        const selectPlayers = async (action) => {
        ...
        createInteractionCollector({
            ...
            message,
            onClick: (action) => {
                if (action.customId === customIds.deselect)
            ...
            onSelect: (action) => {
                if (action.customId === customIds.categorySelect)
                    return selectCategory(action);
                if (Object.values(playerCustomIds).includes(action.customId))
                    return selectPlayers(action);
            },
            onUserSelect(interaction) {
                if (interaction.customId === customIds.user)
                    return selectUser(interaction);
            }
        });
    }
    async changeRoster(interaction, { roster, playerTag, user, categoryId, rosterId }) {
        ...
        const categorySelectButton = new ButtonBuilder()
            ...
            .setCustomId(customIds.category)
            .setStyle(ButtonStyle.Secondary);
        const buttonRow = new ActionRowBuilder().addComponents(confirmButton, rosterSelectButton);
        if (categories.length)
            buttonRow.addComponents(categorySelectButton);
        ...
        const chooseRoster = async (action) => {
            const rosterMenu = new StringSelectMenuBuilder()
                .setMinValues(1)
                .setPlaceholder('Select Roster')
                .setCustomId(customIds.rosterSelect)
                .setOptions(rosters.slice(0, 25).map((roster) => ({
                label: roster.name,
        ...
        const selectRoster = async (action) => {
        ...
        createInteractionCollector({
            ...
            onClick: (action) => {
                ...
                if (action.customId === customIds.confirm)
                    return confirm(action);
                if (action.customId === customIds.roster)
                    return chooseRoster(action);
                if (action.customId === customIds.category)
                    return chooseCategory(action);
            },
            onSelect: (action) => {
                ...
                if (Object.values(playerCustomIds).includes(action.customId))
                    return selectPlayers(action);
                if (action.customId === customIds.rosterSelect)
                    return selectRoster(action);
            },
            onUserSelect(interaction) {
                if (interaction.customId === customIds.user)
                    return selectUser(interaction);
    ...
    async delUsers(interaction, { roster, user }) {
        const playerCustomIds = {
            0: this.client.uuid(interaction.user.id),
            1: this.client.uuid(interaction.user.id),
            2: this.client.uuid(interaction.user.id)
        ...
        const deselectButton = new ButtonBuilder()
            ...
            .setCustomId(customIds.deselect)
            .setStyle(ButtonStyle.Danger);
        const buttonRow = new ActionRowBuilder().addComponents(confirmButton, deselectButton);
        const headerTexts = [
        ...
        const selectUsers = async (action) => {
            selected.userIds = action.values;
            selected.user = action.users.first() ?? null;
            const _playerRows = playerRows();
            confirmButton.setDisabled(!selected.playerTags.length);
        ...
        const confirm = async (action) => {
            ...
            if (!updated)
                return action.editReply({ content: 'Roster was deleted.', components: [] });
            if (members.length) {
                this.client.rosterManager.rosterChangeLog({
    ...
    async addUsers(interaction, { roster, user, clan, categoryId }) {
        const playerCustomIds = {
            0: this.client.uuid(interaction.user.id),
            1: this.client.uuid(interaction.user.id),
            2: this.client.uuid(interaction.user.id)
        ...
        const deselectButton = new ButtonBuilder()
            ...
            .setStyle(ButtonStyle.Danger)
            .setDisabled(false);
        const buttonRow = new ActionRowBuilder().addComponents(confirmButton, bulkAddButton);
        if (categories.length)
            buttonRow.addComponents(categorySelectButton);
        buttonRow.addComponents(deselectButton);
        const headerTexts = [
        ...
        const bulkAdd = async (action) => {
            ...
            try {
                ...
                selected.players = await this.client.rosterManager.getClanMemberLinks(unique(playerTags).map((tag) => ({ tag })), true);
        ...
        createInteractionCollector({
            ...
            message,
            onClick: (action) => {
                if (action.customId === customIds.bulk)
                    return bulkAdd(action);
                if (action.customId === customIds.deselect)
                    return deselect(action);
                if (action.customId === customIds.category)
                    return chooseCategory(action);
...
//# sourceMappingURL=roster-manage.js.map