...
export default class RosterSignupCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        players.sort((a, b) => b.townHallLevel ** (b.townHallWeaponLevel ?? 1) -
        ...
        const categoryMenu = new StringSelectMenuBuilder()
            ...
            .setOptions(selectableCategories.map((category) => ({
        ...
        const accountsRows = cluster(_options.slice(0, 75), 25).map((options, idx) => {
            ...
            return new ActionRowBuilder().addComponents(accountsMenu);
        });
        const msg = await interaction.followUp({
            ...
            flags: MessageFlags.Ephemeral,
            components: args.signup && roster.allowCategorySelection && selectableCategories.length
        ...
        const selectCategory = async (action) => {
            ...
            categoryMenu.setOptions(selectableCategories.map((category) => ({
                label: category.displayName,
                value: category._id.toHexString(),
                default: selected.category === category._id.toHexString()
            })));
...
//# sourceMappingURL=roster-signup.js.map