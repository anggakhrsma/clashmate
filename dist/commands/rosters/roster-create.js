...
import { DEFAULT_TROPHY_ROSTER_LAYOUT, rosterLayoutMap } from '../../struct/roster-manager.js';
...
export default class RosterCreateCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const data = {
            ...
            rosterImage: args.roster_image_url && URL_REGEX.test(args.roster_image_url)
        ...
        if (args.category === 'NO_CLAN')
            data.maxMembers = 500;
        if (args.roster_role) {
            const dup = await this.client.rosterManager.rosters.findOne({ roleId: args.roster_role.id, closed: false }, { projection: { _id: 1 } });
            if (dup)
...
//# sourceMappingURL=roster-create.js.map