...
import { rosterClan, rosterLabel, rosterLayoutMap } from '../../struct/roster-manager.js';
...
export default class RosterEditCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const closeRoster = async (action) => {
            const updated = await this.client.rosterManager.close(rosterId);
            if (!updated)
                return action.reply({ content: 'Roster was deleted.', flags: MessageFlags.Ephemeral });
        ...
        const openRoster = async (action) => {
            if (roster.endTime && new Date(roster.endTime) < new Date()) {
                return action.reply({
                    content: 'This roster cannot be opened as the closing time has passed.',
                    flags: MessageFlags.Ephemeral
                });
            }
        ...
        const selectClear = async (action) => {
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
                .setCustomId(customIds.clear)
                .setLabel('Confirm')
        ...
        const getLink = async (action) => {
            const token = this.client.util.createToken({
                userId: interaction.user.id,
                guildId: interaction.guild.id
            });
            const url = `https://clashperk.com/rosters?roster=${roster._id.toHexString()}&bot=${'public'}&token=${token}`;
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
                .setURL(url)
                .setLabel('Manage Roster on the Web')
        ...
        const clearRoster = async (action) => {
        ...
        const archiveRoster = async (action) => {
            ...
            const { res, body: clan } = roster.clan
                ? await this.client.coc.getClan(roster.clan.tag)
                : { res: null, body: null };
        };
        const toggleSignup = async (action) => {
            await action.update({
        ...
        const selectLayout = async (action) => {
            ...
            const updated = await this.client.rosterManager.edit(rosterId, {
                layout: selected.layoutIds.join('/')
            });
            if (!updated)
                return action.reply({ content: 'Roster was deleted.', flags: MessageFlags.Ephemeral });
            await action.deferUpdate();
        ...
        const editRoster = async (action) => {
            ...
            const sortMenu = new StringSelectMenuBuilder()
                ...
                .setOptions(sortingItems.map((option) => ({
                label: option.name,
                value: option.value,
                default: roster.sortBy === option.value
            })));
        ...
        createInteractionCollector({
            ...
            message,
            onClick: (action) => {
                if (!this.client.util.isManager(action.member, "rosterManagerRole" /* Settings.ROSTER_MANAGER_ROLE */)) {
                    return action.reply({
                        flags: MessageFlags.Ephemeral,
                        content: this.i18n('common.missing_manager_role', { lng: action.locale })
                    });
                }
                if (action.customId === customIds.clear)
                    return clearRoster(action);
            },
            onSelect: async (action) => {
                const value = action.values.at(0);
                if (!this.client.util.isManager(action.member, "rosterManagerRole" /* Settings.ROSTER_MANAGER_ROLE */) &&
                    !this.client.util.isManager(action.member, "rosterManagerRole" /* Settings.ROSTER_MANAGER_ROLE */)) {
                    return action.reply({
                        flags: MessageFlags.Ephemeral,
                        content: this.i18n('common.missing_manager_role', { lng: action.locale })
                    });
...
//# sourceMappingURL=roster-settings.js.map