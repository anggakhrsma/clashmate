import { CAPITAL_LEAGUE_MAP, UNRANKED_CAPITAL_LEAGUE_ID, UNRANKED_WAR_LEAGUE_ID, WAR_LEAGUE_MAP } from '../../util/constants.js';
...
export default class SummaryLeaguesCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const row = new ActionRowBuilder().setComponents(new ButtonBuilder()
    ...
    getCapitalLeagueId(clan) {
        return clan.capitalLeague?.id ?? UNRANKED_CAPITAL_LEAGUE_ID;
    }
    getWarLeagueGroups(guild, clans) {
        const leagueGroup = Object.entries(clans.reduce((acc, clan) => {
    ...
    async getCapitalLeagueGroups(guild, clans) {
        const leagueGroup = Object.entries(clans.reduce((acc, clan) => {
        ...
        const leaguesMap = leagues.reduce((acc, league) => {
            if (league._clanCapitalPoints && league.clanCapitalPoints) {
                const emoji = league._capitalLeague.id > league.capitalLeague.id
                    ? EMOJIS.UP_KEY
    ...
    extraSpace(len, index) {
        return index === len - 1 ? '' : '\n\u200b';
    }
}
//# sourceMappingURL=summary-leagues.js.map