...
export default class SummaryCWLRanks extends Command {
    ...
    async exec(interaction, args) {
        ...
        for (const clan of _clans) {
            ...
            if (!leagueGroup?.leagues?.[clan.tag] || leagueGroup.season !== season)
                continue;
            const isApiData = !(!lastLeagueGroup.res.ok ||
        ...
        const leagueGroups = Object.entries(chunks.reduce((acc, cur) => {
            acc[cur.warLeagueId] ??= [];
        ...
        if (!chunks.length) {
            return interaction.editReply(this.i18n('command.cwl.no_season_data', {
...
//# sourceMappingURL=summary-cwl-ranks.js.map