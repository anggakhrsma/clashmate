...
export default class SummaryMissedWarsCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        for (const { tag } of clans) {
            const wars = await this.client.db
                ...
                state: 'warEnded',
                warType: args.war_type === 'regular-and-cwl'
                    ? { $in: [1 /* WarType.REGULAR */, 3 /* WarType.CWL */] }
            ...
            for (const war of wars) {
                ...
                for (const m of clan.members) {
                    ...
                    mem.wars += 1;
                    if (m.attacks?.length === war.attacksPerMember)
                        continue;
    ...
    getEmbed(members, season) {
        const [content] = Util.splitMessage([
            '\u200e # MISS WARS  NAME',
            ...members
                .slice(0, 99)
                .map((m, i) => `\u200e${this.pad(i + 1, 2)} ${this.pad(m.missed)} ${this.pad(m.wars)}  ${m.name}`)
        ].join('\n'), { maxLength: 4000 });
...
//# sourceMappingURL=summary-missed-wars.js.map