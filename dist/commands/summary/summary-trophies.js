...
export default class SummaryTrophiesCommand extends Command {
    ...
    async exec(interaction, args) {
        let limit = 99;
        if (args.limit)
            limit = Math.max(5, Math.min(99, args.limit));
        ...
        const members = _clans
            .map((clan) => clan.memberList.map((mem) => ({
            clan: clan.name,
            name: mem.name,
            tag: mem.tag,
        ...
        const grouped = Object.values(_clans.reduce((acc, clan) => {
            acc[clan.tag] = {
                name: clan.name,
                tag: clan.tag,
        ...
        if (args.clans_only) {
            ...
            const players = (await Promise.all(memberTags.map((tag) => this.client.coc.getPlayer(tag))))
                .filter((r) => r.res.ok)
                .map((r) => r.body);
...
//# sourceMappingURL=summary-trophies.js.map