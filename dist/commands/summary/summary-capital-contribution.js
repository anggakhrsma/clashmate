    ...
    async exec(interaction, args) {
        ...
        if (args.clans_only) {
            ...
            embed.setDescription([
                ...
                clansGroup
                    .map((clan, i) => `${padStart(i + 1, 2)}  ${padStart(clan.total, maxPad)}  ${clan.name}`)
                    .join('\n'),
                '```'
            ].join('\n'));
        }
            ...
            embed
                ...
                '\u200e #  TOTAL  NAME',
                membersGroup
                    .map((mem, i) => `\u200e${padStart(i + 1, 2)}  ${this.padding(mem.total)}  ${mem.name}`)
                    .join('\n'),
                '```'
            ].join('\n'));
        }
...
//# sourceMappingURL=summary-capital-contribution.js.map