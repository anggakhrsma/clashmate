import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, StringSelectMenuBuilder, embedLength } from 'discord.js';
...
export default class DonationSummaryCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        await Promise.allSettled(fetched.map((clan) => recoverDonations(clan, season)));
        ...
        const orderingRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    async clanDonations(interaction, { clans, seasonId, sortBy, orderBy }) {
        ...
        if (aggregated.length) {
            embed.setDescription([
                ...
                Util.splitMessage(aggregated
                    .map((clan, n) => {
                    return `${BLUE_NUMBERS[++n]} \`\u200e${this.donation(clan.donations, clanDp)} ${this.donation(clan.donationsReceived, clanRp)}  ${Util.escapeBackTick(clan.name).padEnd(15, ' ')}\u200f\``;
                })
                    .join('\n'), { maxLength: 4000 })[0]
            ].join('\n'));
        }
    ...
    async playerDonations(interaction, { clans, seasonId, sortBy, orderBy }) {
        const orders = [];
        for (const key of sortBy ?? ['donations'])
            orders.push({ $sort: { [key]: orderBy === 'asc' ? 1 : -1 } });
...
//# sourceMappingURL=summary-donations.js.map