...
export default class AutoRoleListCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const allowNonFamilyTownHallRoles = this.client.settings.get(interaction.guild, "allowExternalAccounts" /* Settings.ALLOW_EXTERNAL_ACCOUNTS */, false);
        const allowNonFamilyLeagueRoles = this.client.settings.get(interaction.guildId, "allowExternalAccountsLeague" /* Settings.ALLOW_EXTERNAL_ACCOUNTS_LEAGUE */, false);
        ...
        const _clanRolesAggregated = Object.values(rolesMap.clanRoles ?? {})
            ...
            .reduce((prev, { role, roleId }) => {
            if (role && roleId && role in _rolesPriority) {
                prev[role] ??= { role, roleIds: [] };
                if (!prev[role].roleIds.includes(roleId))
                    prev[role].roleIds.push(roleId);
            }
            return prev;
        }, {});
        const _clanRoles = Object.values(_clanRolesAggregated);
        const warRoles = Array.from(new Set(Object.values(rolesMap.clanRoles ?? {})
        ...
        if (args.expand && clanRoleList.length) {
            ...
            embed.addFields({
                name: 'EOS Push Role',
                value: rolesMap.eosPushClans.length && rolesMap.eosPushClanRoles
        ...
        const useAutoRole = this.client.settings.get(interaction.guild, "useAutoRole" /* Settings.USE_AUTO_ROLE */, true);
        const requiresVerification = this.client.settings.get(interaction.guildId, "verifiedOnlyClanRoles" /* Settings.VERIFIED_ONLY_CLAN_ROLES */, false);
        const footerTexts = [];
...
//# sourceMappingURL=autorole-list.js.map