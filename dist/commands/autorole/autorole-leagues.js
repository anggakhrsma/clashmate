import { PLAYER_LEAGUE_NAMES, TROPHY_ROLES } from '../../util/constants.js';
...
export default class AutoLeagueRoleCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        const leagueRolesConfig = this.client.settings.get(interaction.guildId, "leagueRoles" /* Settings.LEAGUE_ROLES */, {});
        const trophyRolesConfig = this.client.settings.get(interaction.guildId, "trophyRoles" /* Settings.TROPHY_ROLES */, {});
        Object.assign(leagueRolesConfig, Object.fromEntries(selectedLeagueRoles.map((selected) => [selected.league, selected.role.id])));
        Object.assign(trophyRolesConfig, Object.fromEntries(selectedTrophyRoles.map((selected) => [
            selected.key,
            { key: selected.key, min: selected.min, max: selected.max, roleId: selected.role.id }
        ])));
        await this.client.settings.set(interaction.guildId, "leagueRoles" /* Settings.LEAGUE_ROLES */, leagueRolesConfig);
        await this.client.settings.set(interaction.guildId, "trophyRoles" /* Settings.TROPHY_ROLES */, trophyRolesConfig);
...
//# sourceMappingURL=autorole-leagues.js.map