import { ActionRowBuilder, EmbedBuilder, MessageFlags, RoleSelectMenuBuilder } from 'discord.js';
...
export default class AutoFamilyRoleCommand extends Command {
    ...
    async exec(interaction, args) {
        ...
        if (args.family_role) {
            await this.client.settings.set(interaction.guild, "familyRole" /* Settings.FAMILY_ROLE */, args.family_role.id);
        }
        if (args.exclusive_family_role) {
            await this.client.settings.set(interaction.guild, "exclusiveFamilyRole" /* Settings.EXCLUSIVE_FAMILY_ROLE */, args.exclusive_family_role.id);
        }
        if (args.family_leaders_role) {
            const familyLeadersRoles = this.client.settings.get(interaction.guild, "familyLeadersRole" /* Settings.FAMILY_LEADERS_ROLE */, []);
            await this.client.settings.set(interaction.guild, "familyLeadersRole" /* Settings.FAMILY_LEADERS_ROLE */, unique([...familyLeadersRoles, args.family_leaders_role.id]).filter((id) => interaction.guild.roles.cache.has(id)));
        }
        if (args.guest_role) {
            await this.client.settings.set(interaction.guild, "guestRole" /* Settings.GUEST_ROLE */, args.guest_role.id);
        }
        if (args.verified_role) {
            await this.client.settings.set(interaction.guild, "accountVerifiedRole" /* Settings.ACCOUNT_VERIFIED_ROLE */, args.verified_role.id);
        }
        if (args.account_linked_role) {
            await this.client.settings.set(interaction.guild, "accountLinkedRole" /* Settings.ACCOUNT_LINKED_ROLE */, args.account_linked_role.id);
        }
        ...
        const familyLeadersRoles = this.client.settings
            .get(interaction.guild, "familyLeadersRole" /* Settings.FAMILY_LEADERS_ROLE */, [])
            .filter((id) => interaction.guild.roles.cache.has(id));
        if (familyLeadersRoles.length)
            menu.setDefaultRoles(...familyLeadersRoles);
        ...
        createInteractionCollector({
            ...
            onRoleSelect: async (action) => {
                ...
                await this.client.settings.set(action.guild, "familyLeadersRole" /* Settings.FAMILY_LEADERS_ROLE */, action.roles.map((role) => role.id));
                const embed = await mutate();
...
//# sourceMappingURL=autorole-family.js.map