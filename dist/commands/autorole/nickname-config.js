import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, DiscordjsError, DiscordjsErrorCodes, escapeMarkdown, ModalBuilder, SectionBuilder, SeparatorSpacingSize, TextDisplayBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
...
export default class NicknameConfigCommand extends Command {
    ...
    async exec(interaction, args) {
        let familyFormat = this.client.settings.get(interaction.guildId, "familyNicknameFormat" /* Settings.FAMILY_NICKNAME_FORMAT */, '');
        if (args.family_nickname_format && familyFormat !== args.family_nickname_format)
            familyFormat = args.family_nickname_format;
        let nonFamilyFormat = this.client.settings.get(interaction.guildId, "nonFamilyNicknameFormat" /* Settings.NON_FAMILY_NICKNAME_FORMAT */, '');
        ...
        if (args.family_nickname_format && !/^none$/i.test(args.family_nickname_format)) {
            if (/{NAME}|{PLAYER_NAME}|{DISCORD_NAME}|{DISCORD_USERNAME}|{USERNAME}|{DISCORD}/gi.test(familyFormat)) {
                this.client.settings.set(interaction.guildId, "familyNicknameFormat" /* Settings.FAMILY_NICKNAME_FORMAT */, familyFormat);
            }
            else {
                return interaction.editReply(`Invalid **family nickname** format \`${familyFormat}\`, it must include \`{PLAYER_NAME}\` or \`{DISCORD_NAME}\` or \`{DISCORD_USERNAME}\``);
            }
        }
        if (args.family_nickname_format && /^none$/i.test(args.family_nickname_format)) {
            this.client.settings.set(interaction.guildId, "familyNicknameFormat" /* Settings.FAMILY_NICKNAME_FORMAT */, '');
        }
        if (args.non_family_nickname_format && !/^none$/i.test(args.non_family_nickname_format)) {
            if (/{CLAN}|{CLAN_NAME}|{ALIAS}|{CLAN_ALIAS}|{ALIASES}|{CLAN_ALIASES}|{ROLE}|{CLAN_ROLE}/gi.test(nonFamilyFormat)) {
                return interaction.editReply(`Invalid **non-family nickname** format \`${nonFamilyFormat}\`, it must **not** include \`{CLAN}\` \`{CLAN_NAME}\` \`{ALIAS}\` \`{CLAN_ALIAS}\` \`{ROLE}\` \`{CLAN_ROLE}\``);
            }
            else if (!/{NAME}|{PLAYER_NAME}|{DISCORD_NAME}|{DISCORD_USERNAME}|{USERNAME}|{DISCORD}/gi.test(nonFamilyFormat)) {
                return interaction.editReply(`Invalid **non-family nickname** format \`${nonFamilyFormat}\`, it must include \`{PLAYER_NAME}\` or \`{DISCORD_NAME}\` or \`{DISCORD_USERNAME}\``);
            }
            else {
                this.client.settings.set(interaction.guildId, "nonFamilyNicknameFormat" /* Settings.NON_FAMILY_NICKNAME_FORMAT */, nonFamilyFormat);
            }
        }
        if (args.non_family_nickname_format && /^none$/i.test(args.non_family_nickname_format)) {
            this.client.settings.set(interaction.guildId, "nonFamilyNicknameFormat" /* Settings.NON_FAMILY_NICKNAME_FORMAT */, '');
        }
        if (typeof args.change_nicknames === 'boolean') {
            await this.client.settings.set(interaction.guildId, "autoNickname" /* Settings.AUTO_NICKNAME */, Boolean(args.change_nicknames));
        }
        const enabledAuto = this.client.settings.get(interaction.guildId, "autoNickname" /* Settings.AUTO_NICKNAME */, false);
        const autoRole = this.client.settings.get(interaction.guildId, "useAutoRole" /* Settings.USE_AUTO_ROLE */, false);
        if (args.account_preference_for_naming) {
            await this.client.settings.set(interaction.guildId, "nicknamingAccountPreference" /* Settings.NICKNAMING_ACCOUNT_PREFERENCE */, args.account_preference_for_naming);
        }
        const accountPreference = this.client.settings.get(interaction.guildId, "nicknamingAccountPreference" /* Settings.NICKNAMING_ACCOUNT_PREFERENCE */, NicknamingAccountPreference.DEFAULT_OR_BEST_ACCOUNT);
        const state = this.client.settings.get(interaction.guild, "roleReplacementLabels" /* Settings.ROLE_REPLACEMENT_LABELS */, {});
        ...
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(settings.map((config) => `**${config.name}**\n${config.value}`).join('\n')));
        ...
        createInteractionCollector({
            ...
            onClick: async (action) => {
                ...
                const leaderInput = new TextInputBuilder()
                    ...
                    .setRequired(false);
                if (state.leader)
                    leaderInput.setValue(state.leader);
                const coLeaderInput = new TextInputBuilder()
                    ...
                    .setRequired(false);
                if (state.coLeader)
                    coLeaderInput.setValue(state.coLeader);
                const elderInput = new TextInputBuilder()
                    ...
                    .setRequired(false);
                if (state.admin)
                    elderInput.setValue(state.admin);
                const memberInput = new TextInputBuilder()
                    ...
                    .setRequired(false);
                if (state.member)
                    memberInput.setValue(state.member);
                modal.addComponents(new ActionRowBuilder().addComponents(leaderInput), new ActionRowBuilder().addComponents(coLeaderInput), new ActionRowBuilder().addComponents(elderInput), new ActionRowBuilder().addComponents(memberInput));
                ...
                try {
                    ...
                    mutateLabelsText();
                    await this.client.settings.set(interaction.guild, "roleReplacementLabels" /* Settings.ROLE_REPLACEMENT_LABELS */, state);
...
//# sourceMappingURL=nickname-config.js.map