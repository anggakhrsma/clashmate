...
const getSheetValue = (value) => {
    ...
    if (value instanceof Date)
        return { numberValue: Util.dateToSerialDate(value) };
    return {};
};
const getUserEnteredFormat = (value) => {
    if (value instanceof Date)
...
export const createHyperlink = (url, text) => `=HYPERLINK("${url}","${text}")`;
const getConditionalFormatRequests = (sheets) => {
    const gridStyleRequests = sheets
        .map((sheet, sheetId) => [
        {
...
const getStyleRequests = (sheets) => {
    const styleRequests = sheets
        .map(({ columns }, sheetId) => columns
...
const createSheetRequest = async (title, sheets) => {
...
const createColumnRequest = (columns) => {
    return {
...
const timezone = async (query) => {
    const location = await getLocation(query);
    if (!location)
        return null;
    const { lat, lon, display_name } = location;
    // Free timezone lookup via timeapi.io
    const tzResult = await fetch(`https://timeapi.io/api/timezone/coordinate?latitude=${lat}&longitude=${lon}`)
        .then((res) => res.json())
        .catch(() => null);
