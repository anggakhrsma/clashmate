...
import { GuildScheduledEventEntityType, GuildScheduledEventPrivacyLevel, PermissionFlagsBits, time } from 'discord.js';
...
export class GuildEventsHandler {
    ...
    get collection() {
        return this.client.db.collection("GuildEvents" /* Collections.GUILD_EVENTS */);
    }
    getEvents(lng, { filtered, useGraceTime }) {
        const now = moment().toDate();
        const graceTime = useGraceTime ? maxGraceTime : 0;
        const clanGamesStartTime = moment(now).date() > 28 || (moment(now).date() >= 28 && moment(now).hour() >= 8)
        ...
        const clanGamesEndTime = moment(clanGamesStartTime)
            ...
            .toDate();
        const cwlStartTime = moment(now).date() > 10 || (moment(now).date() >= 10 && moment(now).hour() >= 8)
        ...
        const getCorrectedEndTime = (timestamp) => {
            timestamp = moment(timestamp).add(graceTime, 'milliseconds').toDate();
            return timestamp;
        };
        const getFormattedValue = (timestamp) => {
        ...
        const events = [
            {
                ...
                visible: moment(now).isBefore(clanGamesStartTime) ||
            ...
            },
            {
                ...
                visible: moment(now).isBefore(raidWeekStartTime) ||
    ...
    getMaxDuration(lng, event, guildEvent) {
        if (guildEvent.durationOverrides?.includes(event.type)) {
            const record = this.getEvents(lng, { filtered: false, useGraceTime: false }).find((ev) => ev.type === event.type.replace(/_start/g, '_end'));
            if (record && record.timestamp > Date.now())
                return record.timestamp;
        }
        return event.timestamp + maxGraceTime;
    }
    async create(guild, guildEvent) {
        ...
        for (const event of this.getEvents(guild.preferredLocale, {
            ...
            if (guildEvent.allowedEvents && !guildEvent.allowedEvents.includes(event.type))
                continue;
            if (event.timestamp === guildEvent.events[event.type])
                continue;
            if (event.timestamp + maxGraceTime === guildEvent.events[event.type])
                continue;
            ...
            await this.collection.updateOne({ guildId: guild.id }, {
                $set: {
    ...
    async init() {
        ...
        try {
            for await (const guildEvent of cursor) {
                ...
                if (!guild || !isEnabled)
                    continue;
                if (this.client.settings.hasCustomBot(guild) && !false)
                    continue;
                try {
...
//# sourceMappingURL=guild-events-handler.js.map