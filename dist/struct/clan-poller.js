 ...
 */
import { LogActions } from '../entities/index.js';
...
export class ClanPoller {
    constructor(client) {
        ...
        Object.defineProperty(this, "clanSnapshots", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Collection()
        });
        Object.defineProperty(this, "warSnapshots", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Collection()
        });
        Object.defineProperty(this, "playerCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Collection()
        });
        Object.defineProperty(this, "clanTimer", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "warTimer", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "playerTimer", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
    ...
    // ── Helpers ──────────────────────────────────────────────────────────────
    async getTrackedTags() {
        const clans = await this.client.db
            .collection("ClanStores" /* Collections.CLAN_STORES */)
            .find({ guild: { $in: [...this.client.guilds.cache.keys()] }, paused: { $ne: true } }, { projection: { tag: 1 } })
            .toArray();
    ...
    memberEvent(member, op) {
        return {
            op,
            tag: member.tag,
            name: member.name,
            role: member.role,
            leagueId: member.league?.id ?? 29000000,
    ...
    async clanTick() {
        try {
            ...
            if (this.running)
                this.clanTimer = setTimeout(() => this.clanTick(), CLAN_POLL_INTERVAL);
        }
    }
    async pollClan(tag) {
        ...
        const next = { clan, memberMap, lastDonationReset: this.donationWeekKey() };
        const prev = this.clanSnapshots.get(tag);
        if (prev)
            await this.diffClan(prev, next);
        this.clanSnapshots.set(tag, next);
    }
    async diffClan(prev, next) {
        const { clan } = next;
        const members = [];
        // Join / leave
        for (const [tag, member] of next.memberMap) {
            if (!prev.memberMap.has(tag))
                members.push(this.memberEvent(member, LogActions.JOINED));
        }
        for (const [tag, member] of prev.memberMap) {
            if (!next.memberMap.has(tag))
                members.push(this.memberEvent(member, LogActions.LEFT));
        }
        ...
        for (const [tag, curr] of next.memberMap) {
            ...
            if (old.role !== curr.role) {
                ...
                members.push(this.memberEvent(curr, isPromotion ? LogActions.PROMOTED : LogActions.DEMOTED));
            }
            ...
            if (old.name !== curr.name)
                members.push(this.memberEvent(curr, LogActions.NAME_CHANGE));
            if (old.warPreference !== curr.warPreference)
                members.push(this.memberEvent(curr, LogActions.WAR_PREF_CHANGE));
            if (next.lastDonationReset === prev.lastDonationReset &&
            ...
            const clanActivity = old.name !== curr.name ||
                old.townHallLevel !== curr.townHallLevel ||
                old.warPreference !== curr.warPreference ||
        ...
        const clanEvents = [];
        if (prev.clan.clanLevel !== clan.clanLevel)
            clanEvents.push(LogActions.CLAN_LEVEL_UP);
    ...
    async warTick() {
        try {
            ...
            if (this.running)
                this.warTimer = setTimeout(() => this.warTick(), WAR_POLL_INTERVAL);
        }
    }
    async pollWar(tag) {
        ...
        if (['inWar', 'warEnded'].includes(war.state)) {
            await this.client.db
                ...
                .updateOne({ warId: warTag }, {
                $set: {
                    warId: warTag,
                    ...war,
                    warType: war.isFriendly ? 'friendly' : 'random',
                    updatedAt: new Date()
                },
    ...
    async playerTick() {
        try {
            ...
            for (const [, snap] of this.clanSnapshots) {
                for (const tag of snap.memberMap.keys())
                    memberTags.add(tag);
            }
            ...
            if (memberTags.size === 0) {
                ...
                for (const clanTag of tags) {
                    const { res, body } = await this.client.coc
                        .getClan(clanTag)
                        .catch(() => ({ res: { ok: false }, body: null }));
                    if (res.ok && body?.memberList) {
                        for (const m of body.memberList)
                            memberTags.add(m.tag);
                    }
                }
            }
    ...
    async pollPlayer(tag) {
        ...
        const ach = (name) => player.achievements?.find((a) => a.name === name)?.value ?? 0;
        const curr = {
            // Direct fields
            ...
            activeSuperTroops: (player.troops ?? []).filter((t) => t.superTroopIsActive).length
        };
        this.playerCache.set(tag, curr);
        if (!prev)
            return; // first poll — no diff yet
        ...
        await Promise.all([
            ...
            ...this.getClanTagsForPlayer(tag).map((clanTag) => this.client.db
                .collection('PlayerActivities')
                .insertOne({
                tag,
                name: player.name,
    ...
    getClanTagsForPlayer(playerTag) {
        const clanTags = [];
        for (const [clanTag, snap] of this.clanSnapshots) {
            if (snap.memberMap.has(playerTag))
                clanTags.push(clanTag);
        }
        return clanTags;
    }
}
//# sourceMappingURL=clan-poller.js.map