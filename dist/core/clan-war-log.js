import { FeatureFlags, calculateCWLMedals } from '../util/constants.js';
import { ClanLogType } from '../entities/index.js';
import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, WebhookClient, escapeMarkdown, time } from 'discord.js';
...
export class ClanWarLog extends RootLog {
    ...
    async handleMessage(cache, webhook, data) {
        ...
        if (data.type === 'CWL_ENDED') {
            if (cache.logType !== ClanLogType.CWL_MONTHLY_SUMMARY_LOG)
                return null;
        ...
        if (data.state === 'warEnded' &&
            ...
            if (data.warTag && cache.logType !== ClanLogType.CWL_MISSED_ATTACKS_LOG)
                return null;
            if (!data.warTag && cache.logType !== ClanLogType.WAR_MISSED_ATTACKS_LOG)
                return null;
            const isEnabled = this.client.isFeatureEnabled(FeatureFlags.ALLOW_NO_MISSED_ATTACKS_LOG, cache.guild);
            if (!isEnabled && !data.remaining.length)
                return null;
            const embed = this.getRemaining(data);
            return this.send(cache, webhook, { embeds: [embed], threadId: cache.threadId });
        }
        // LINEUP CHANGE LOG
        if ((data.oldMembers?.length || data.newMembers?.length) &&
            cache.logType === ClanLogType.CWL_LINEUP_CHANGE_LOG) {
            const embed = this.getLineupChangeEmbed(data);
            return this.send(cache, webhook, { embeds: [embed], threadId: cache.threadId });
        }
        // WAR ATTACK LOG
        if ((data.newAttacks?.length || data.newDefenses?.length) &&
            [ClanLogType.WAR_ATTACK_LOG, ClanLogType.CWL_ATTACK_LOG].includes(cache.logType)) {
            if (data.warTag && cache.logType !== ClanLogType.CWL_ATTACK_LOG)
                return null;
            if (!data.warTag && cache.logType !== ClanLogType.WAR_ATTACK_LOG)
                return null;
            const content = this.getAttackLogMessage(data);
            return this.send(cache, webhook, { content, threadId: cache.threadId });
        }
        if (data.warTag && cache.logType !== ClanLogType.CWL_EMBED_LOG)
            return null;
        if (!data.warTag && cache.logType !== ClanLogType.WAR_EMBED_LOG)
            return null;
    ...
    _components(data) {
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
            ...
            .setCustomId(JSON.stringify({ cmd: 'war', war_id: data.id, tag: data.clan.tag, attacks: true })), new ButtonBuilder()
            .setLabel('Defenses')
    ...
    async mutate(cache, data, message) {
        if (!message) {
            if (cache.message)
        ...
        if (data.warTag) {
            ...
            return this.collection.updateOne({ _id: cache._id }, {
                $set: {
                    updatedAt: new Date(),
                    failed: 0,
    ...
    embed(data) {
        if (data.warTag)
            return this.getLeagueWarEmbed(data);
                ...
                embed.addFields([
                    {
                        ...
                        value: [
                            ...data.recent.map(({ attacker, defender }) => {
                                ...
                                const destruction = Math.floor(attacker.destructionPercentage)
                                    .toString()
                                    .concat('%')
                                    .padStart(pad, ' ');
                                return `${stars} \`\u200e${destruction}\` ${BLUE_NUMBERS[attacker.mapPosition]}${ORANGE_NUMBERS[attacker.townHallLevel]}${EMOJIS.VS}${BLUE_NUMBERS[defender.mapPosition]}${ORANGE_NUMBERS[defender.townHallLevel]} ${name}`;
                            })
                        ].join('\n')
                    }
                ]);
    ...
    getRemaining(data) {
        ...
        if (twoRem.length) {
            ...
            embed.addFields(chunks.map((chunk, i) => ({
    ...
    getAttackLogMessage(data) {
        return [...data.newAttacks, ...data.newDefenses]
            ...
            const attackerMap = BLUE_NUMBERS[isClanMember ? attacker.mapPosition : attacker.defender.mapPosition];
            const defenderMap = BLUE_NUMBERS[isClanMember ? attacker.defender.mapPosition : attacker.mapPosition];
            const attackerTh = ORANGE_NUMBERS[isClanMember ? attacker.townhallLevel : attacker.defender.townhallLevel];
            const defenderTh = ORANGE_NUMBERS[isClanMember ? attacker.defender.townhallLevel : attacker.townhallLevel];
            const vs = isClanMember ? WAR_STARS.ARROW_RIGHT : WAR_STARS.ARROW_LEFT;
            return `${stars} \`${destruction}\` ${attackerMap}${attackerTh} \u200e${name} ${vs}${defenderMap}${defenderTh}`;
        })
            .join('\n');
    }
    ...
    getStars(oldStars, newStars, isDefense = false) {
...
//# sourceMappingURL=clan-war-log.js.map