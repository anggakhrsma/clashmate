...
export default class GuildMemberUpdateListener extends Listener {
    ...
    async exec(newMember) {
        const config = this.client.settings.get(newMember.guild_id, "reminderExclusion" /* Settings.REMINDER_EXCLUSION */, { type: 'optIn' });
        ...
        let updated = false;
        if (config.wars) {
            if ((newMember.roles.includes(config.wars) &&
        ...
        if (config.games) {
            if ((newMember.roles.includes(config.games) &&
        ...
        if (config.raids) {
            if ((newMember.roles.includes(config.raids) &&
        ...
        if (updated) {
            await this.client.settings.set(newMember.guild_id, "reminderExclusion" /* Settings.REMINDER_EXCLUSION */, config);
        }
    }
}
//# sourceMappingURL=guild-member-update.js.map