...
// not in use
export const linkListEmbedMaker = async ({ clan, guild, showTag }) => {
    ...
    const notLinked = clan.memberList.filter((m) => !notInDiscord.some((en) => en.tag === m.tag) &&
    ...
    const chunks = Util.splitMessage([
        ...
        notInDiscord
            ...
            const name = escapeBackTick(mem.name).padEnd(15, ' ');
            const member = clan.memberList.find((m) => m.tag === mem.tag);
            const user = member.tag.padStart(12, ' ');
            return { name, user, verified: mem.verified };
        })
            .sort((a, b) => localeSort(a.name, b.name))
            .map(({ name, user, verified }) => {
...
//# sourceMappingURL=link-list.helper.js.map