...
export function mapToPlayerInterface(player) {
    ...
    return {
        ...
        heroEquipment: UNITS_LIST.filter((unit) => RAW_UNITS_MAP[unit.name]?.category === 'equipment' && player.units?.[unit.id]).map((unit) => {
            const raw = RAW_UNITS_MAP[unit.name];
            return {
                name: unit.name,
                level: player.units[unit.id],
        ...
        heroes: UNITS_LIST.filter((unit) => RAW_UNITS_MAP[unit.name]?.category === 'hero' && player.units?.[unit.id]).map((hero) => {
            ...
            return {
                ...
                maxLevel: raw.maxLevel,
                village: raw.village,
                equipment: UNITS_LIST.filter((equip) => RAW_UNITS_MAP[equip.name]?.category === 'equipment' &&
                    player.heroes?.[hero.id]?.equipment[equip.name]).map((unit) => {
                    const raw = RAW_UNITS_MAP[unit.name];
                    return {
                        name: unit.name,
                        level: player.units[unit.id],
...
const ACHIEVEMENTS_MAP_BY_NAME = {
    'Bigger Coffers': 1,
    'Get those Goblins!': 2,
    'Bigger & Better': 3,
    'Nice and Tidy': 4,
...
export const UNITS_MAP_BY_NAME = {
    'Barbarian': 1,
    'Archer': 2,
    'Goblin': 3,
    'Giant': 4,
...
const UNITS_LIST = Object.entries(UNITS_MAP_BY_NAME).reduce((record, [name, id]) => {
    record.push({ name, id: `${id}` });
    return record;
}, []);
...
//# sourceMappingURL=cache-mapper.helper.js.map