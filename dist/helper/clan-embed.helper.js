import { UNRANKED_CAPITAL_LEAGUE_ID } from '../util/constants.js';
...
export const clanEmbedMaker = async (clan, { description, accepts, color, bannerImage, fields }) => {
    if (!fields || fields?.includes('*'))
        fields = Object.values(ClanEmbedFields);
    ...
    if (fields?.includes(ClanEmbedFields.REQUIREMENTS)) {
        embed.addFields({
            name: 'Requirements',
            value: `${EMOJIS.TOWN_HALL} ${!accepts || accepts?.toLowerCase() === 'auto'
    ...
    if (fields?.includes(ClanEmbedFields.TROPHIES_REQUIRED)) {
        embed.addFields({
            ...
            value: `${EMOJIS.TROPHY} ${clan.requiredTrophies}${clan.requiredBuilderBaseTrophies
    ...
    if (fields?.includes(ClanEmbedFields.WAR_PERFORMANCE)) {
        embed.addFields({
            ...
            value: `${EMOJIS.OK} ${clan.warWins} Won${clan.isWarLogPublic
    ...
    if (clan.warFrequency &&
        ...
        embed.addFields({
            name: 'War Frequency',
            value: `${clan.warFrequency.toLowerCase() === 'morethanonceperweek'
    ...
    if (fields?.includes(ClanEmbedFields.CLAN_CAPITAL_LEAGUE)) {
        embed.addFields({
            ...
            value: `${CAPITAL_LEAGUES[clan.capitalLeague?.id ?? UNRANKED_CAPITAL_LEAGUE_ID]} ${clan.capitalLeague?.name ?? 'Unranked'} ${EMOJIS.CAPITAL_TROPHY} ${clan.clanCapitalPoints || 0}`
        });
    }
...
//# sourceMappingURL=clan-embed.helper.js.map