  ...
  public async exec(
    ...
    if (!members.length && data?.members) {
      ...
      members = [...data.members]
        .map((m: any) => ({ ...m, attackLimit: m.attackLimit + (m.bonusAttackLimit ?? 0) }))
        .sort((a: any, b: any) => b.capitalResourcesLooted - a.capitalResourcesLooted);
      // Add clan members who didn't participate
      clan.memberList.forEach((member) => {
        if (!members.find((m: any) => m.tag === member.tag)) {
          members.push({
            name: member.name,
            tag: member.tag,
