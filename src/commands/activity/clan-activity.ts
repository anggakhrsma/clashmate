...
export default class ClanActivityCommand extends Command {
  ...
  public async exec(
    ...
    const labels = dataLabel.map((d) => {
      ...
      if (!isHourly) return m.format('MMM D');
      // At midnight (12am), show date instead of time
      if (m.hour() === 0) return m.format('MMM D');
    ...
    const getNickname = async (tag: string, name: string) => {
      const nickname = await this.client.storage.getNickname(interaction.guildId, tag, name);
      return nickname || name;
    };
    const legendLabels = await Promise.all(
      result.map((r) => getNickname(r.clanTag, clansMap[r.clanTag] || r.clanTag))
    );
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            grid: {
              color: (ctx: any) => (ctx.index % 2 === 0 ? '#2a2a3e' : 'transparent')
            }
          },
          y: {
            beginAtZero: true,
  ...
  private async aggregate(clanTags: string[], days: number, limit: number) {
    ...
    const rows = await this.client.db
      ...
      .aggregate<{ clanTag: string; time: string; count: number }>([
        {
          $match: {
            clanTag: { $in: clanTags },
            createdAt: { $gte: since }
    ...
    >((acc, row) => {
      if (!acc[row.clanTag]) acc[row.clanTag] = { activities: [], total: 0 };
  ...
  private buildDataset(
    dataLabel: { key: string }[],
    clan: { activities: { count: number; time: string }[] }
  ) {
