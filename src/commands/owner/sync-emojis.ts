...
export default class SyncEmojisCommand extends Command {
  ...
  public async exec(interaction: CommandInteraction) {
    ...
    const emojiMap = new Map<string, string>(); // name → <:name:id>

    for (const guild of this.client.guilds.cache.values()) {
      try {
        // Fetch emojis if not cached
        const emojis =
          guild.emojis.cache.size > 0 ? guild.emojis.cache : await guild.emojis.fetch();

        for (const emoji of emojis.values()) {
          if (!emoji.name || emoji.animated) continue;
    ...
    await interaction.editReply(
      `✅ Found **${emojiMap.size}** unique emojis across **${this.client.guilds.cache.size}** servers.\n🔄 Updating emojis.ts...`
    );
