...
export default class UnitsCommand extends Command {
  ...
  public async exec(
    ...
    const options = players.map((op: any) => ({
      description: op.tag,
      label: op.name,
      value: op.tag,
  ...
  private embed(data: APIPlayer, showMaxLevel = false, equipmentOnly = false) {
    ...
    const unlockedEquipment = unique([
      ...data.heroes.flatMap((u: any) => u.equipment ?? []).map((u: any) => u.name),
      ...data.heroEquipment.map((u: any) => u.name)
    ]);
    const Troops = RAW_TROOPS_WITH_ICONS.filter(
      (troop: any) => !troop.seasonal && !(troop.name in SUPER_TROOPS)
    )
      .filter((unit: any) => {
    ...
    for (const category of filteredUnits.sort((a: any, b: any) => a.index - b.index)) {
      ...
      if (unitsArray.length) {
        ...
        chunkedUnitsArray.forEach((chunk: any, index: any) => {
          embed.addFields([
            {
    ...
    const superTroops = RAW_SUPER_TROOPS.filter((unit: any) =>
      apiTroops.find(
        (un: any) =>
      ...
      const { maxLevel, level, name } = apiTroops.find(
        (u: any) => u.name === unit.original && u.village === unit.village
      ) ?? {
        maxLevel: 0,
        level: 0
      };
      ...
      const originalTroop = RAW_TROOPS_WITH_ICONS.find(
        (un: any) => un.name === name && un.category === 'troop' && un.village === 'home'
      );
    ...
    const activeSuperTroops = data.troops
      .filter((en: any) => en.superTroopIsActive)
      .map((en: any) => en.name);
