...
export default class BoostsCommand extends Command {
  ...
  public async exec(
    ...
    args: { tag?: string; unit?: string; recent?: boolean; user?: User }
  ) {
    ...
    const players = members.filter(
      (mem: any) => mem.troops.filter((en: any) => en.superTroopIsActive).length
    );
    if (!players.length)
      return interaction.followUp({
    ...
    const boostTimes = await this.client.db
      ...
      .find(
        { tag: { $in: players.map((m: any) => m.tag) } },
        { projection: { _id: 0, tag: 1, lastSeen: 1 } }
      )
      .toArray();

    const recently = boostTimes
      .filter((m: any) => m.lastSeen >= new Date(Date.now() - 10 * 60 * 1000))
      .map((m: any) => m.tag);

    const selected = players
      .filter(
        (mem: any) =>
          mem.troops.filter((en: any) => en.name === args.unit && en.superTroopIsActive).length
      )
      .filter((m: any) =>
        recently.length && args.recent ? recently.includes(m.tag) : true
      ).length;

    const boosters = players.filter((m: any) =>
      recently.length && args.recent ? recently.includes(m.tag) : true
    );
    const memObj = boosters.reduce<{
      [key: string]: { name: string; duration: number; online: boolean }[];
    }>((pre: any, curr: any) => {
      for (const troop of curr.troops) {
        if (
          troop.name in SUPER_TROOPS &&
          troop.superTroopIsActive &&
          (args.unit && selected ? args.unit === troop.name : true)
        ) {
          if (!(troop.name in pre)) pre[troop.name] = [];
          pre[troop.name].push({
            name: curr.name,
            duration: 0,
            online: recently.includes(curr.tag)
          });
        }
      }
      return pre;
      ...
      embed.addFields([
        {
          ...
          value: Util.splitMessage(
            `${val
              .map(
                (mem: any) =>
                  `\u200e${mem.name}${mem.duration ? ` (${Util.duration(mem.duration)})` : ''} ${mem.online ? EMOJIS.ONLINE : ''}`
              )
              .join('\n')}\n\u200b`,
            { maxLength: 1024 }
          )[0]
    ...
    if (args.unit && !selected) {
      return interaction.followUp({
        flags: MessageFlags.Ephemeral,
        content: args.recent
          ? this.i18n('command.boosts.no_recent_unit_boosts', {
    ...
    const menus = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
      new StringSelectMenuBuilder()
        ...
        .addOptions(
          Object.entries(SUPER_TROOPS).map(([key, value]) => ({
            label: key,
            value: key,
            emoji: value
          }))
  ...
  private boostable(players: APIPlayer[]) {
    return players
      .filter((en: any) => en.townHallLevel >= 11)
      .reduce((pre: any, curr: any) => {
        const troops = RAW_SUPER_TROOPS.filter((unit: any) =>
          curr.troops.find(
            (un: any) =>
              un.village === 'home' &&
