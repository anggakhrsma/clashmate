...
export default class UpgradesCommand extends Command {
  ...
  public embed(data: APIPlayer, equipmentOnly?: boolean) {
    ...
    for (const category of units.sort((a: any, b: any) => a.index - b.index)) {
      ...
      const totalMaxLevel = unitsArray.reduce(
        (prev: any, curr: any) => prev + curr.hallMaxLevel,
        0
      );
      const totalLevel = unitsArray.reduce((prev: any, curr: any) => prev + curr.level, 0);
      const remaining = `${Math.round((totalLevel * 100) / totalMaxLevel)}%`;

      const costPerResource = (unitsArray as any[]).reduce<Record<string, number>>(
        (prev: any, curr: any) => {
          prev[curr.resource] ??= 0;
