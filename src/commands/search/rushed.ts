...
export default class RushedCommand extends Command {
  ...
  private embed(data: APIPlayer, locale: string) {
    ...
    const Troops = RAW_TROOPS_WITH_ICONS.filter((unit: any) => {
    ...
    const TroopsObj = Troops.reduce<TroopJSON>((prev: any, curr: any) => {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        [
          ...
          '```\u200eTH   LAB  HERO  NAME',
          members
            .sort((a: any, b: any) => Number(b.rushed.overall) - Number(a.rushed.overall))
            .map(
              (en: any) =>
  ...
  private totalPercentage(hallLevel: number, rushed: number) {
    const totalTroops = RAW_TROOPS_FILTERED.filter(
      (unit: any) => unit.village === 'home' && unit.levels[hallLevel - 2] > 0
    );
