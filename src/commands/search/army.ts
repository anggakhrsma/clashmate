...
export default class ArmyCommand extends Command {
  ...
  public embed(
    ...
    const malformed = ![...troopList, ...spellList].every(
      (en: any) =>
        typeof en.id === 'number' && typeof en.total === 'number' && en.total <= TOTAL_UNITS
    );
    if (malformed)
      return { embeds: [], content: this.i18n('command.army.invalid_link', { lng: locale }) };
    ...
    const findUnits = ({
      ...
      return parts
        .filter((parts: any) => _findOne(parts.id))
        .map((parts: any) => {
    ...
    const superTroops = troopList
      .filter((parts: any) =>
        RAW_SUPER_TROOPS.find((en: any) => en.id === parts.id && en.name in SUPER_TROOPS)
      )
      .map((parts: any) => {
        const unit = RAW_SUPER_TROOPS.find(
          (en: any) => en.id === parts.id && en.name in SUPER_TROOPS
        )!;
        return {
          id: parts.id,
          total: parts.total,
          ...
          subCategory: 'super',
          hallLevel:
            RAW_TROOPS.find((en: any) => en.name === unit.original)!.levels.findIndex(
              (en: any) => en >= unit.minOriginalLevel
            ) + 1,
          housing: unit.housingSpace
        };
      });
    ...
    const heroes = heroList
      .filter((parts: any) =>
        RAW_TROOPS.find(
          (en: any) => en.id === parts.id && en.category === 'hero' && en.name in HOME_HEROES
        )
      )
      .map((parts: any) => {
        ...
        return {
          ...
          equipment: findUnits({
            category: 'equipment',
            parts: parts.equipment.map((i: any) => ({
              id: i,
              total: 1
            })),
    ...
    const hallByUnlockTH = Math.max(
      ...troops.map((en: any) => en.hallLevel),
      ...spells.map((en: any) => en.hallLevel),
      ...siegeMachines.map((en: any) => en.hallLevel),
      ...superTroops.map((en: any) => en.hallLevel)
    );

    const [totalTroop, totalSpell] = [
      troops.reduce((pre: any, cur: any) => pre + cur.housing * cur.total, 0) +
        superTroops.reduce((pre: any, curr: any) => pre + curr.housing * curr.total, 0),
      spells.reduce((pre: any, cur: any) => pre + cur.housing * cur.total, 0),
      siegeMachines.reduce((pre: any, cur: any) => pre + cur.housing * cur.total, 0)
    ];

    const hallByTroops =
      ARMY_CAPACITY.find((en: any) => en.troops >= Math.min(totalTroop, TOTAL_UNITS))?.hall ?? 0;
    const hallBySpells =
      ARMY_CAPACITY.find((en: any) => en.spells >= Math.min(totalSpell, TOTAL_SPELLS))?.hall ?? 0;
      ...
      embed.setDescription(
        [
          ...
          heroes
            .map((en: any) => {
              const exts = en.pets
                .map((eq: any) => `${EMOJIS.GAP} ${HERO_PETS[eq.name]} ${eq.name}`)
                .concat(
                  ...en.equipment.map(
                    (eq: any) => `${EMOJIS.GAP} ${HERO_EQUIPMENT[eq.name]} ${eq.name}`
                  )
                )
                .join('\n');
    ...
    if (troops.length) {
      embed.setDescription(
        [
          ...
          '**Troops**',
          troops
            .map((en: any) => `\u200e\`${this.padding(en.total)}\` ${TROOPS[en.name]}  ${en.name}`)
            .join('\n')
        ].join('\n')
      );
    }
      ...
      embed.addFields([
        {
          ...
          value: [
            '**Spells**',
            spells
              .map((en: any) => `\u200e\`${this.padding(en.total)}\` ${SPELLS[en.name]} ${en.name}`)
              .join('\n')
          ].join('\n')
        }
      ]);
      ...
      embed.addFields([
        {
          ...
          value: [
            ...
            superTroops
              .map(
                (en: any) =>
                  `\u200e\`${this.padding(en.total)}\` ${SUPER_TROOPS[en.name]}  ${en.name}`
              )
              .join('\n')
          ].join('\n')
        }
      ...
      embed.addFields([
        {
          ...
          value: [
            ...
            siegeMachines
              .map(
                (en: any) =>
                  `\u200e\`${this.padding(en.total)}\` ${SIEGE_MACHINES[en.name]}  ${en.name}`
              )
              .join('\n')
          ].join('\n')
        }
