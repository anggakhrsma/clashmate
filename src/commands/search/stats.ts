...
export default class StatsCommand extends Command {
  ...
  private compare(value: string): Compare {
    ...
    const match = /^(?<attackerTownHall>\d{1,2})(vs?|\s+)(?<defenderTownHall>\d{1,2})$/i.exec(
      value
    );
  ...
  private async getDataSource(
    ...
    const data = await this.client.resolver.resolveClan(
      interaction,
      args.tag ?? interaction.user.id
    );
    if (!data) return null;
  ...
  public async exec(
    ...
    if (!stats.length) {
      return interaction.editReply(
        this.i18n('command.stats.no_stats', { lng: interaction.locale })
      );
    }
