...
export class ClanLog extends RootLog {
  ...
  private async getPlayerLogEmbed(cache: Cache, member: Member, data: Feed) {
    ...
    if (member.op === LogActions.JOINED) {
      ...
      const heroes = player.heroes.filter((hero: any) => hero.village === 'home');
      embed.setDescription(
        [
          `${TOWN_HALLS[player.townHallLevel]}**${player.townHallLevel}**`,
          `${HOME_BASE_LEAGUES[player.leagueTier?.id ?? UNRANKED_TIER_ID]!}**${player.trophies}**`,
  ...
  private async getClanLogEmbed(cache: Cache, webhook: WebhookClient, data: Feed) {
    ...
    if (data.clan.badge) embed.setThumbnail(data.clan.badge);
  ...
  private async getDonationLogEmbed(cache: Cache, webhook: WebhookClient, data: Feed) {
    ...
    if (data.clan.badgeUrl) embed.setThumbnail(data.clan.badgeUrl);
    if (data.clan.badgeUrl) embed.setFooter({ text: `${data.clan.members}/50`, iconURL: data.clan.badgeUrl });
    else embed.setFooter({ text: `${data.clan.members}/50` });

    embed.setTimestamp();
    embed.setColor(COLOR_CODES.PURPLE);

    const donatingMembers = data.members.filter((m: any) => m.op === LogActions.DONATED);
    ...
    const receivingMembers = data.members.filter((m: any) => m.op === LogActions.RECEIVED);
  ...
  private formatHeroes(heroes: APIPlayerItem[]) {
    return heroes.length
      ? `${heroes.map((hero: any) => `${HEROES[hero.name]}**${hero.level}**`).join(' ')}`
      : ``;
  }
  ...
  public async init() {
    const guildIds = this.client.guilds.cache.map((guild: any) => guild.id);
