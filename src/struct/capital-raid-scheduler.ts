...
export class CapitalRaidScheduler {
  ...
  private queue(schedule: RaidSchedulersEntity) {
    if (this.client.settings.hasCustomBot(schedule.guild) && !false) return;
  ...
  public async unwantedMembers(clanMembers: { tag: string }[], weekId: string, clanTag: string) {
    const res: unknown[] = []; // redis removed
    const members = (res as any[]).filter((m) => m.weekId === weekId && m.clan.tag !== clanTag);
    return (members as any[]).map((m) => m.tag);
  }
    ...
    try {
      ...
      if (this.wasInMaintenance(schedule, data)) {
        this.client.logger.info(
          `Raid reminder shifted [${schedule.tag}] ${schedule.timestamp.toISOString()} => ${moment(data.endTime).toDate().toISOString()}`,
          { label: 'REMINDER' }
        );
  ...
  private async _refresh() {
    ...
    for await (const schedule of cursor) {
      ...
      if (this.client.settings.hasCustomBot(schedule.guild) && !false) continue;
