...
export const getLegendRankingEmbedMaker = async ({
  ...
  const _clans = (await Promise.all(clanTags.map((t: string) => client.coc.getClan(t))))
    .filter((r: any) => r.res.ok)
    .map((r: any) => r.body);
  const memberTags = _clans.map((clan) => clan.memberList.map((member: any) => member.tag)).flat();
  const _players = (await Promise.all(memberTags.map((t: string) => client.coc.getPlayer(t))))
    .filter((r: any) => r.res.ok)
    .map((r: any) => r.body);
