...
export function mapToPlayerInterface(player: PartialPlayer): APIPlayer {
  ...
  return {
    ...
    clan: player.clan
      ...
      : undefined,
    achievements: ACHIEVEMENT_LIST.filter((ach: any) => player.achievements?.[ach.id] >= 0).map(
      ({ name, id }) => ({
        name,
    ...
    troops: UNITS_LIST.filter(
      (unit: any) => RAW_UNITS_MAP[unit.name]?.category === 'troop' && player.units?.[unit.id]
    ).map((unit: any) => {
    ...
    spells: UNITS_LIST.filter(
      (unit: any) => RAW_UNITS_MAP[unit.name]?.category === 'spell' && player.units?.[unit.id]
    ).map((unit: any) => {
      const raw = RAW_UNITS_MAP[unit.name];
      return {
        name: unit.name,
    ...
    heroEquipment: UNITS_LIST.filter(
      (unit: any) => RAW_UNITS_MAP[unit.name]?.category === 'equipment' && player.units?.[unit.id]
    ).map((unit: any) => {
      const raw = RAW_UNITS_MAP[unit.name];
      return {
        name: unit.name,
    ...
    heroes: UNITS_LIST.filter(
      (unit: any) => RAW_UNITS_MAP[unit.name]?.category === 'hero' && player.units?.[unit.id]
    ).map((hero: any) => {
...
const RAW_UNITS_MAP = RAW_DATA.RAW_UNITS.reduce<Record<string, RawUnit>>((record: any, unit: any) => {
  record[unit.name] = {
    id: unit.id,
    name: unit.name,
