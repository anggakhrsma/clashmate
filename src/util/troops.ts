...
export const RAW_TROOPS_FILTERED = RawData.RawUnits.filter((unit: any) => !unit.seasonal)
  .filter((u: any) => u.category !== 'equipment')
  .filter((unit: any) => !(unit.name in SUPER_TROOPS) && unit.name in ALL_TROOPS);

export const RAW_TROOPS_WITH_ICONS = RawData.RawUnits.filter((unit: any) => !unit.seasonal)
  // .filter((u: any) => u.category !== 'equipment')
  .filter((unit: any) => !(unit.name in SUPER_TROOPS) && unit.name in ALL_TROOPS);
