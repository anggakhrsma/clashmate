...
export default class AutocompleteInteractionListener extends Listener {
  ...
  public async autocomplete(interaction: AutocompleteInteraction) {
    ...
    switch (focused) {
      ...
      case 'location':
        return this.client.autocomplete.locationAutocomplete(
          interaction,
          interaction.options.getString(focused) ?? undefined
        );
    }
  }
  ...
  private async playerTagAutocomplete(interaction: AutocompleteInteraction, focused: string) {
    ...
    return interaction.respond(
      players.map((p) => ({ value: p.tag, name: `${p.name ?? p.tag} (${p.tag})` }))
    );
  }

  /** MongoDB-backed clan tag autocomplete (replaces Elasticsearch). */
  private async clanTagAutocomplete(
  ...
  private async rosterAutocomplete(
    interaction: AutocompleteInteraction<'cached'>,
    focused: string,
    allowBulk: boolean
  ) {
    const filter: Filter<IRoster> = { guildId: interaction.guild.id };
    const query = interaction.options.getString(focused)?.trim();
    if (query) {
      const text = query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    ...
    if (allowBulk) options.unshift({ value: '*', name: 'All Rosters (Bulk Edit)' });
    return interaction.respond(options);
  }

  private async rosterCategoryAutocomplete(
    interaction: AutocompleteInteraction<'cached'>,
    focused: string
  ) {
    const filter: Filter<IRosterCategory> = { guildId: interaction.guild.id };
    const query = interaction.options.getString(focused)?.trim();
    if (query) {
      const text = query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  ...
  private async aliasAutoComplete(interaction: AutocompleteInteraction<'cached'>, focused: string) {
    ...
    const aliases = clans
      .filter(
        (clan) => clan.alias && (query ? new RegExp(`.*${query}.*`, 'i').test(clan.alias) : true)
      )
      .slice(0, 24);

    if (!aliases.length) return interaction.respond([{ value: '0', name: 'No aliases found.' }]);
    return interaction.respond(
      aliases.map((clan) => ({ value: clan.alias!, name: `${clan.alias!} - ${clan.name}` }))
    );
  }
  ...
  private async durationAutocomplete(interaction: AutocompleteInteraction, focused: string) {
    ...
    const matchedDur =
      dur?.match(/\d+?\.?\d+?[dhm]|\d[dhm]/g)?.reduce((acc, cur) => acc + ms(cur), 0) ?? 0;
