...
class BaseHandler extends EventEmitter {
  ...
  public async register() {
    for await (const dir of readdirp(this.directory, {
      ...
      const mod = container.resolve<Command | Listener | Inhibitor>(imported.default);
      this.construct(mod);
    }
  }
...
export class CommandHandler extends BaseHandler {
  ...
  private async handleComponentInteraction(interaction: MessageComponentInteraction) {
    ...
    let parsed: CustomIdProps = { cmd: '', users: [] };
    try {
      const raw = interaction.customId;
      // First check our payload store (from createId)
      const stored = (this.client as any)._componentPayloads?.get(raw);
      if (stored) {
        parsed = stored as CustomIdProps;
      } else if (raw.startsWith('{')) {
        parsed = JSON.parse(raw) as CustomIdProps;
      } else {
    ...
    function resolveMenu(interaction: StringSelectMenuInteraction, parsed: CustomIdProps) {
      ...
      if (parsed.array_key) return { [parsed.array_key as string]: values };
      if (parsed.string_key) return { [parsed.string_key as string]: values.at(0) };
